# alakazam741_v11_board_depth — board-depth and secondary-route revision.
#
...
# energy-aware backup ETA. New logic measures independent Alakazam route depth rather than the
# old raw "three line bodies" cap, develops more deeply when going second, treats Fezandipiti as
# a real fallback attacker/draw engine, deploys Shaymin only into bench-pressure, and permits
# Enhanced Hammer as tempo only when removing a Special Energy leaves a two-energy deficit.
...
from policy_base import (
    BasePolicy, ATTACK_COST_ENERGIES, BENCH_DAMAGE_ATTACKS, EFFECT_PREVENT_ENERGY,
    ENERGY_PROVIDES, ITEM_LOCK_IDS, attack_table, card_table, get_card, make_agent, new_diag,
    prize_count,
)
...
RECOVERABLE = (C.ABRA, C.KADABRA, C.ALAKAZAM, C.DUNSPARCE, C.DUDUNSPARCE, C.SHAYMIN,
               C.PSYCHIC_ENERGY)
...
class AlakazamPolicy(BasePolicy):
    ...
    def _have_attacker(self):
        return any(p is not None and p.id in ATTACKER_IDS and self.can_attack(p)
                   for p in self.my_board())

    def _ready_secondary_attacker(self):
        return any(p is not None and p.id == C.FEZANDIPITI_EX and self.can_attack(p)
                   for p in self.my_board())
    ...
    def _is_second_player(self):
        first = getattr(self.state, "firstPlayer", self.my_index)
        return first in (0, 1) and first != self.my_index
    ...
    def _desired_route_depth(self):
        # Going second was the largest observed weakness.  Build three represented routes during
        # the opening, and also when a bench-pressure deck is present; otherwise two is enough.
        if (self._is_second_player() and self._early_game()) or self.opponent_threatens_bench():
            return 3
        if self.field[C.ALAKAZAM] > 0:
            return 3 if len(self.me.prize) >= 4 and self._early_game() else 2
        return 2
    ...
    def _needs_more_abra_body(self):
        ...
        return self._open_bench() and bodies < self._desired_route_depth()

    def _shaymin_worthwhile(self):
        if self.field[C.SHAYMIN] > 0 or not self._open_bench():
            return False
        if not self.opponent_threatens_bench():
            return False
        vulnerable = [p for p in self.me.bench if p is not None and p.id != C.SHAYMIN]
        # Do not spend a Powerful Hand card to protect an empty/unimportant bench.
        return any(p.id in ALAKAZAM_LINE or p.id in ENGINE_LINE for p in vulnerable)

    def _fezandipiti_worthwhile(self):
        if self.field[C.FEZANDIPITI_EX] > 0 or not self._open_bench():
            return False
        # Fez is a two-prize liability, so deploy it only when it supplies a missing role: recovery
        # draw after prizes have been taken, a fallback attack route, or opening board depth while
        # going second.
        prize_lost = len(self.me.prize) < 6
        no_attack_route = not self._ready_alakazam_attacker() and self._backup_eta() > 1
        second_player_opening = self._is_second_player() and self._early_game()
        return prize_lost or no_attack_route or second_player_opening
    ...
    def _improves_plan(self, option):
        ...
        if t == OptionType.PLAY:
            ...
            if (self._needs_first_backup() or self._needs_more_abra_body()) and self._open_bench():
    ...
    def _optional_deck_spend(self, option):
        ...
        if option.type == OptionType.ABILITY:
            ...
            if card.id == C.DUDUNSPARCE and option.area == AreaType.BENCH:
                return self._optional_spend_ok(
                    ...
                    secures_backup=self._needs_first_backup() or self._needs_route_depth())
        ...
        if option.type != OptionType.PLAY:
            return False
        cid = self._play_card_id(option)
        if cid not in (C.BUDDY_POFFIN, C.POKE_PAD, C.HILDA, C.DAWN, C.HYPER_AROMA):
            return False
        opp = self.opponent.active[0] if self.opponent.active else None
    ...
    def _action_tier(self, option, state):
        ...
        if t == OptionType.PLAY:
            ...
            if cid in (C.NIGHT_STRETCHER, C.SACRED_ASH) and state == TurnState.RECOVER:
                return Tier.BUILD_ATTACKER
            if cid in (C.BUDDY_POFFIN, C.POKE_PAD, C.HILDA, C.DAWN, C.HYPER_AROMA):
                return Tier.SEARCH
            if cid == C.NIGHT_STRETCHER and self._night_stretcher_worthwhile():
                return Tier.BUILD_ATTACKER if not self._ready_alakazam_attacker() else Tier.BUILD_BACKUP
    ...
    def _hammer_tempo_targets(self):
        """Special Energy cards whose removal leaves every attack at least two attachments away.

        A one-energy deficit is not a true one-turn stop: the opponent can simply attach next turn.
        Requiring a deficit of two avoids returning to the old "Hammer because one exists" logic.
        """
        active = self.opponent.active[0] if self.opponent.active else None
        if active is None or not self._has_meaningful_attack_option():
            return set()
        ...
        targets = set()
        for e in (getattr(active, "energyCards", None) or []):
            ed = card_table.get(getattr(e, "id", None))
            if ed is None or ed.cardType != CardType.SPECIAL_ENERGY:
                continue
            post = list(attached)
            provided = ENERGY_PROVIDES.get(e.id, EnergyType.COLORLESS)
            if provided in post:
                post.remove(provided)
            elif post:
                post.pop()
            deficits = [self._energy_deficit(post, ATTACK_COST_ENERGIES.get(aid, []))
                        for aid in data.attacks if aid in ATTACK_COST_ENERGIES]
            if deficits and min(deficits) >= 2:
                targets.add(e.id)
        return targets

    def _enhanced_hammer_worthwhile(self):
        """Unlock Powerful Hand, or create a concrete two-energy tempo gap before our attack."""
        opp = self.opponent.active[0] if self.opponent.active else None
        if opp is None:
            return False
        if self._opp_active_has_prevent_energy():
            return self._ready_alakazam_attacker() and 20 * max(0, self.me.handCount - 1) > 0
        return bool(self._hammer_tempo_targets())
    ...
    def _battle_cage_worthwhile(self):
        ...
        """
        if self.state.stadiumPlayed or self.stadium_id == C.BATTLE_CAGE or self.field[C.SHAYMIN] > 0:
            return False
        bench = [p for p in self.me.bench if p is not None]
    ...
    def _score_ability(self, o):
        ...
        if card is None:
            return 0
        if card.id == C.FEZANDIPITI_EX:
            # The engine only offers Flip the Script when its KO condition is met.  Use it as a
            # hand-positive recovery action, but keep the same dynamic deckout discipline as Dudun.
    ...
    def _score_play_poke(self, card):
        ...
        if cid == C.ABRA:
            if not self._needs_more_abra_body():
                return 1800
            # Going second, an extra independent Abra before the first attack is worth more than
            # the old raw-body cap allowed.  Diminishing returns remain to protect hand size.
            second_bonus = 1800 if self._is_second_player() and self._early_game() else 0
            return 20500 + second_bonus - 350 * n
        if cid == C.DUNSPARCE:
            engine_count = self.field[C.DUNSPARCE] + self.field[C.DUDUNSPARCE]
            target = 2 if not self._is_second_player() else 1
            if engine_count >= target:
                return 1200
            return 18500 - 250 * n
        if cid == C.SHAYMIN:
            return 19800 if self._shaymin_worthwhile() else 300
        if cid == C.FEZANDIPITI_EX:
            return 18800 if self._fezandipiti_worthwhile() else 700
        return 14000 - 200 * n
    ...
    def _score_attach(self, o):
        ...
        if p.id == C.FEZANDIPITI_EX:
            # BasePolicy deliberately rejects partial multi-energy funding.  Fez is the one explicit
            # exception: it is our fallback route, but it must not steal fuel from the first Alakazam.
            current = self.energy_count(p)
            if current >= 3 or src is None:
                return -1
            if not self._ready_alakazam_attacker() and self._backup_eta() > 1:
                return -1
            if self._backup_energy_short():
                return -1
            base = 8500 if current == 2 else 1700 if current == 1 else 700
        else:
    ...
    def _score_card(self, o):
        ...
        if o.playerIndex == self.op_index and not isinstance(card, Pokemon):
            ...
            if getattr(context_card, "id", None) == C.ENHANCED_HAMMER:
                ...
                if card.id in self._hammer_tempo_targets():
                    return 1900 + active_bonus
        ...
        if ctx in (SelectContext.DAMAGE_COUNTER, SelectContext.DAMAGE_COUNTER_ANY):
            if isinstance(card, Pokemon) and o.playerIndex == self.op_index:
                ...
                return 10000 + ko_bonus + prize_count(card) * 1000 - hp
            return 0
    ...
    def _score_attach_target(self, p, is_active):
        ...
        if p.id in (C.ABRA, C.KADABRA):
            return 1500
        if p.id == C.FEZANDIPITI_EX and self.energy_count(p) < 3:
            return 1200 + 250 * self.energy_count(p)
        return -1
    ...
    def _score_active_choice(self, o, card):
        ...
        if card.id in ALAKAZAM_IDS:
            ...
            score += 150 if self.can_attack(card) else 20
    ...
    def _score_to_bench(self, card):
        ...
        n = self.field[cid]
        if cid == C.ABRA:
            return (260 if self._needs_more_abra_body() else 80) - 25 * n
        if cid == C.DUNSPARCE:
            return 140 - 30 * n
        if cid == C.SHAYMIN:
            return 300 if self._shaymin_worthwhile() else 5
        if cid == C.FEZANDIPITI_EX:
            return 240 if self._fezandipiti_worthwhile() else 10
        return 100 - 20 * n
    ...
    def _score_discard(self, card):
        ...
        if self.hand[cid] >= 2:
            return 60
        if cid in ALAKAZAM_LINE or cid in ENGINE_LINE or cid in (C.FEZANDIPITI_EX, C.SHAYMIN):
