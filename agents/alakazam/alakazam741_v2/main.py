# alakazam741_v2 - v1 + 実ラダー対戦ログ(sub54520948)とプレイヤー知見に基づく改善:
#   (1) ドローソースの動的な上限/下限(デッキ切れ負け防止, 残サイド連動フロア)
#   (2) クセロシキのたくらみ x1 投入(-1ボス): ミラーのPowerful Hand対策
#   (3) エンリッチエネルギー vs 超エネルギーの貼り分け基準を明確化
#   (4) ケーシィ(Abra)は進化札を握っている時だけ前に出す
#   (5) グッズをサポートより先に使うシーケンス補正
# Base: wmh/ptcg-abc agents/alakazam v3 (Majkel1337 divergence-mined). Self-contained.
...
class AlakazamPolicy:
    ...
    def _deck_floor(self):
        ...
        残りサイド数 ≈ 勝ち切りに必要なターン数 + 2枚のバッファ。"""
        return max(5, len(self.me.prize) + 2)
    ...
    def _score_play_trainer(self, card):
        ...
        if cid == C.XEROSIC:
            ...
            mirror = any(p.id in (C.ABRA, C.KADABRA, C.ALAKAZAM, C.ALAKAZAM_PSY)
                         for p in opp_board)
            if mirror and opp_hand >= 7:
                return 15500        # 20×(手札-3超分)を丸ごと削る — ドローより優先
            if opp_hand >= 9:
                return 13000
            if mirror and opp_hand >= 5:
                return 9000
    ...
    def _score_active_choice(self, o, card):
        ...
        if card.id in ALAKAZAM_IDS:
            ...
            # 手札に進化札(ユンゲラー / フーディン+ふしぎなアメ)がある時だけ優先する。
