...
def test_active_dudunsparce_may_cycle_when_a_ready_body_remains():
    ...
    try:
        ...
        # v26 supersedes the v11 expectation of 14000 here. Repositioning off a
        # two-body board leaves a single Pokemon, and 88497435 t5 lost exactly
        # this way. The cycle is refused unless the body can be replaced.
        assert obj._score_ability(option) == -1
        obj._offered_body_add = lambda: True
...
def test_v10_shaymin_pivot_pays_one_energy_then_retreats_to_ready_alakazam():
    ...
    # v27 supersedes the old flat 17,700 setup score: the full
    # attach -> retreat -> Alakazam KO route is terminally evaluated.
...
def test_shaymin_is_only_benched_against_attack_spread():
    ...
    # v26 tightened the trigger: Flower Curtain must cover at least two exposed
    # bodies and must not take the bench slot a line body still needs, so the
    # board here also carries an Active and a second exposed Kadabra.
    second = Pokemon(policy.C.KADABRA, hp=80, maxHp=80)
    active = Pokemon(policy.C.ALAKAZAM, hp=140, maxHp=140,
                     energies=[EnergyType.PSYCHIC])
    obj = bare_policy(policy, active=active, bench=[exposed, second],
                      opp_active=grimmsnarl)
    assert obj._opp_threatens_bench()
    assert obj._score_play_poke(shaymin) == 17000

    thin = bare_policy(policy, bench=[exposed], opp_active=grimmsnarl)
    assert not thin._opp_threatens_bench()
