...
def test_v1_ml_scope_is_memory_first_v29_residual_and_safety_guarded():
    source = (Path(__file__).resolve().parent / "policy_runtime.py").read_text(encoding="utf-8")
    main_source = (Path(__file__).resolve().parent / "main.py").read_text(encoding="utf-8")
    assert "ALAKAZAM_ML_V1_ENABLE_OVERRIDE" in source
    ...
    assert "ALAKAZAM_ML_V1_THRESHOLD" in main_source
