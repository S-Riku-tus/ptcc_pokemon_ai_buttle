...
def test_v4_rmy_expert_artifact_matches_primary_feature_contract():
    runtime = HybridRanker()
    assert runtime.rmy_type_model is not None
    assert runtime.rmy_type_model["format"] == "v4_xgb_rmy_type_expert_v1"
...
def test_v4_prediction_is_exact_probability_blend():
    runtime = HybridRanker()
    row = _row(runtime)
...
def test_v4_uses_selected_validation_threshold_and_reports_expert():
    runtime = HybridRanker()
    assert math.isclose(float(runtime.type_model["threshold"]), 0.45)
    snapshot = runtime.snapshot()
    assert snapshot["rmy_type_model_loaded"] is True
    assert snapshot["runtime_scope"].startswith("v4_rmy_blended")
