...
class AlakazamPolicy:
    ...
    def _boss_target_score(self, target):
        ...
        active_reachable_ko = bool(
            active is not None and self._boss_active_reachable_damage(active) >= active.hp
        )
        active_dominates = active_ko or active_reachable_ko
        target_prizes = prize_count(target)
        active_prizes = prize_count(active) if active_dominates and active is not None else 0
        ...
        active_value = self._target_value(active) if active_dominates else 0
        # Never replace an available or deterministically reachable Active KO with
        # an equal/worse Bench KO.
        if active_dominates:
            # If the Active already closes the game, Boss is pure waste.
            if active_prizes >= len(self.me.prize):
                _DIAG["boss_active_ko_dominance_blocks"] += 1
                return -1
            # Prize dominance is absolute. A role bonus never justifies abandoning
            # a 2/3-prize KO for a 1-prize target.
            if target_prizes < active_prizes:
                _DIAG["boss_active_ko_dominance_blocks"] += 1
                if active_reachable_ko and not active_ko:
                    _DIAG["boss_reachable_active_ko_blocks"] += 1
                return -1
            if not winning and target_prizes == active_prizes:
                role_upgrade = (
                    target_role >= active_role + 3000
                    and target_value >= active_value + 1500
                )
                if not role_upgrade:
                    _DIAG["boss_active_ko_dominance_blocks"] += 1
                    return -1
    ...
    def _score(self, o):
        ...
        if t == OptionType.END:
            ...
            return -1000 if self._active_alakazam_attack_route_offered() else 0
        return 0
    ...
    def _enriching_attach_score(self, pokemon, *, is_active=False):
        """Attach Enriching Energy as a recyclable draw engine, never as fake {P}."""
        if pokemon is None or not self._deck_spend_ok(cost=4, allow_lethal=False):
            return -1
        # Rich is Colorless. Spending the once-per-turn attachment on it while a
        # Psychic Energy can fuel the Active Alakazam reproduces six concrete v13
        # missed-attack turns, so the immediate attack route has absolute priority.
        if self._active_alakazam_can_be_fueled():
            return -1

        cycle_ready = self._enrich_cycle_ready(pokemon)
        if cycle_ready:
            # If we can immediately use Run Away Draw, protect enough deck for the
            # combined 4+3 draw sequence. The three cards on the Dudunsparce stack
            # return afterward, but the draws must still be legal first.
            if not self._deck_spend_ok(cost=7, allow_lethal=False):
                return -1
            score = 24800 + (1200 if is_active and self._bench_attacker_ready() else 0)
            return score

        draw_needed = self._enrich_draw_needed()
        need_psychic_now = self._need_p_fuel()

        if pokemon.id == C.DUNSPARCE:
            # Stage it now for a future attach→evolve→Run Away Draw recycle.
            return 15800 if draw_needed else 9800
        if pokemon.id == C.FEZANDIPITI_EX:
            mode = self._fez_mode(pokemon)
            if mode == "PIVOT" and is_active:
                return 23200
            if mode == "ALTERNATE_ATTACKER":
                return 17800 + self._energy_count(pokemon) * 500
            return 9200 if draw_needed and not need_psychic_now else -1
        if pokemon.id in (C.SHAYMIN, C.ABRA, C.KADABRA, C.ALAKAZAM):
            return 7600 if draw_needed and not need_psychic_now else -1
        return 6400 if draw_needed and not need_psychic_now else -1
    ...
    def _score_attack(self, o):
        ...
        if aid == POWERFUL_HAND and self._effect_prevented(opp):
            # The attack is harmless but free. Keep it below all useful setup and
            # Boss/Hammer escapes, yet above END so the turn never idles. This also
            # restores v11's successful last-action behavior.
            return 500
