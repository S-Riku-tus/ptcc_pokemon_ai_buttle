...
def test_v13_enriching_prefers_immediate_dudunsparce_recycle():
    policy = h.load_policy()
    ...
    assert obj._enriching_attach_score(dunsparce) >= 24800
...
def test_v14_powerful_hand_into_mist_is_last_action_fallback():
    policy = h.load_policy()
...
def test_v13_boss_preserves_reachable_grimmsnarl_ko_after_safe_draws():
    policy = h.load_policy()
    mine = h.Pokemon(policy.C.ALAKAZAM, hp=140, maxHp=140,
                     energies=[h.EnergyType.PSYCHIC])
    grimmsnarl = h.Pokemon(policy.GRIMMSNARL_EX_ID, hp=320, maxHp=320, playerIndex=1)
