...
def _diag_template():
    return {
        ...
        "zero_effect_attacks_blocked": 0,
        "hammer_doomed_target_blocks": 0,
        "terminal_boss_blocks": 0,
        "grimmsnarl_bench_blocks": 0,
        "sufficient_damage_draw_blocks": 0,
    }
...
GRIMMSNARL_SIGNATURE_IDS = {646, 647, 648}
BOSS_KEY_ROLE_BONUS = {
...
# Attacks such as Cubchoo's Snotted Up do not appear as a special-condition
# flag.  The engine simply omits ATTACK on the following turn.  Keep the rule
# data-driven so future cards with the same wording receive the same treatment.
...
class AlakazamPolicy:
    ...
    def _turns_to_win(self):
        """Conservative prize-race horizon from the visible board.

        We do not assume an unknown future two-prizer, but an already visible ex
        that our current attacker can affect is a concrete two-prize route.  This
        is intentionally a horizon, not a full search tree.
        """
        remaining = max(0, len(self.me.prize))
        if remaining == 0:
            return 0
        best_prizes = 1
        for pokemon in (self.opponent.active + self.opponent.bench):
            if pokemon is None:
                continue
            if self._active_best_dmg(pokemon) > 0:
                best_prizes = max(best_prizes, prize_count(pokemon))
        return max(1, math.ceil(remaining / max(1, best_prizes)))

    def _turns_to_deckout(self, spend=0):
        """Own turns available before a compulsory draw from an empty deck."""
        return max(0, self.me.deckCount - max(0, spend)) + 1
    ...
    def _deck_preserve(self):
        ...
        if opp is None:
            return False
        remaining_prizes = len(self.me.prize)
        big_hand = 20 * self.me.handCount >= max(opp.hp, 130)  # can essentially KO a body now
        deck_low = self._turns_to_deckout() <= self._turns_to_win() + 4
    ...
    def _deck_spend_ok(self, cost=1, allow_lethal=True):
        ...
        race_safe = self._turns_to_deckout(cost) > self._turns_to_win() + 1
        if self.me.deckCount - cost > self._deck_floor() and race_safe:
            return True
    ...
    def _activate_draw_ok(self):
        ...
        if self._ko_and_continuity_locked():
            _DIAG["sufficient_damage_draw_blocks"] += 1
            return False
    ...
    def _grimmsnarl_matchup(self):
        return bool(self._visible_opponent_ids() & GRIMMSNARL_SIGNATURE_IDS)

    def _munkidori_counter_pressure(self):
        """Visible Adrena-Brain can move up to three counters this turn."""
        damaged_source = any(
            pokemon is not None
            and getattr(pokemon, "hp", 0) < getattr(pokemon, "maxHp", 0)
            for pokemon in (self.opponent.active + self.opponent.bench)
        )
        if not damaged_source:
            return 0
        for pokemon in (self.opponent.active + self.opponent.bench):
            if (pokemon is not None and pokemon.id == 112
                    and EnergyType.DARKNESS in (getattr(pokemon, "energies", None) or [])):
                return 30
        return 0

    def _grim_bench_ko_pressure(self, card_id):
        data = card_table.get(card_id)
        if data is None:
            return False
        projected = self._opponent_ready_bench_damage() + self._munkidori_counter_pressure()
        return projected > 0 and getattr(data, "hp", 0) <= projected

    def _grimmsnarl_bench_allowed(self, card_id):
        """Block redundant bodies only when Shadow Bullet can immediately cash them.

        A fixed four-body cap lost too much setup and draw-engine value in paired
        engine games. Keep the two Alakazam lines and first Dunsparce available;
        suppress only a duplicate fragile body under a concrete 30+30 KO route.
        """
        if not self._grimmsnarl_matchup():
            return True
        if self._board_body_count() <= 1:
            return True
        line = sum(self.field[cid] for cid in
                   (C.ABRA, C.KADABRA, C.ALAKAZAM, C.ALAKAZAM_PSY))
        engine = self.field[C.DUNSPARCE] + self.field[C.DUDUNSPARCE]
        if card_id == C.ABRA and line < 2:
            return True
        if card_id == C.DUNSPARCE and engine < 1:
            return True
        if card_id == C.SHAYMIN and self._opponent_ready_bench_damage() > 0:
            return True
        if card_id == C.FEZANDIPITI_EX and len(self.opponent.prize) <= 2:
            return False
        return not self._grim_bench_ko_pressure(card_id)
    ...
    def _hammer_damage_after_spend(self, target):
        active = self.me.active[0] if self.me.active else None
        if (active is None or target is None or not self._can_attack(active)
                or self._attack_disabled_on_active()):
            return 0
        if active.id == C.ALAKAZAM:
            if self._effect_prevented(target):
                return 0
            spent = 0 if getattr(self._context_effect_card(), "id", None) == C.ENHANCED_HAMMER else 1
            return 20 * max(0, self.me.handCount - spent)
        return self._active_best_dmg(target)

    def _hammer_target_is_doomed(self, energy, pokemon, area):
        """Removing Energy from the Pokémon already being KO'd has zero tempo."""
        if (energy is None or pokemon is None or area != AreaType.ACTIVE
                or energy.id in EFFECT_PREVENT_ENERGY):
            return False
        return self._hammer_damage_after_spend(pokemon) >= getattr(pokemon, "hp", 0)

    def _hammer_target_score(self, energy, pokemon, area):
        """Dedicated Hammer target head: Mist first, concrete tempo second."""
        if energy is None:
            return -1
        if self._hammer_target_is_doomed(energy, pokemon, area):
            _DIAG["hammer_doomed_target_blocks"] += 1
            return -1
    ...
    def _fez_attack_goal(self):
        if self._effect_breaker_required():
            return True
        return any(
    ...
    def _attack_disabled_on_active(self):
        """Whether a previous attack disabled this exact Active Pokémon."""
        active = self.me.active[0] if self.me.active else None
        if active is None:
            return False
        remembered = _V9_STATE.get("attack_disabled_key")
        if remembered is None or remembered != _pokemon_key(active):
            return False
        return not any(
    ...
    def _effect_breaker_required(self):
        """Every visible target rejects Powerful Hand and no immediate Hammer out exists."""
        targets = [
            pokemon for pokemon in (self.opponent.active + self.opponent.bench)
            if pokemon is not None
        ]
        if not targets or not all(self._effect_prevented(pokemon) for pokemon in targets):
            return False
        # An attached Mist/Rock Energy plus Hammer is a one-action unlock, so do
        # that instead of beginning a three-attachment Fezandipiti plan.
        removable = any(
            energy.id in EFFECT_PREVENT_ENERGY
            for _, _, energy in self._attached_special_energy_entries()
        )
        if removable and self.hand[C.ENHANCED_HAMMER] > 0:
            return False
        active = self.me.active[0] if self.me.active else None
        if active is not None and active.id == C.ALAKAZAM_PSY and self._can_attack(active):
            return False
        return True
    ...
    def _boss_active_reachable_damage(self, target):
        """Deterministic Powerful Hand damage still reachable after committing Boss.

        Boss consumes the Supporter and one hand card, but it does not prevent
        evolution draw, Run Away Draw, or an Enriching Energy attachment.  This
        closes the v12 Grimmsnarl→Morgrem error where the Active became a 2-prize
        KO later in the same turn even though Boss had already selected a 1-prizer.
        Only guaranteed hand-count gains are included; no probabilistic search is
        assumed.
        """
        active = self.me.active[0] if self.me.active else None
        if (active is None or active.id != C.ALAKAZAM or target is None
                or not self._can_attack(active) or self._effect_prevented(target)):
            return 0

        hand = max(0, self.me.handCount - (0 if self._boss_resolving() else 1))
        deck = max(0, self.me.deckCount)

        # Existing benched Dudunsparce: draw three, then the stack returns.
        if (deck >= 3 and any(
        ...
        # One deterministic evolution draw route. Alakazam is +2 net; Kadabra or
        # Candy-to-Alakazam is +1 net. We use only a currently evolvable body.
        evolvable_kadabra = any(
            pokemon is not None and pokemon.id == C.KADABRA
            and not getattr(pokemon, "appearThisTurn", False)
            for pokemon in self._my_board()
        )
        evolvable_abra = any(
            pokemon is not None and pokemon.id == C.ABRA
            and not getattr(pokemon, "appearThisTurn", False)
            for pokemon in self._my_board()
        )
        if evolvable_kadabra and self.hand[C.ALAKAZAM] > 0 and deck >= 3:
            hand += 2
            deck -= 3
        elif evolvable_abra and self.hand[C.KADABRA] > 0 and deck >= 2:
            hand += 1
            deck -= 2
        elif (evolvable_abra and self.hand[C.RARE_CANDY] > 0
              and self.hand[C.ALAKAZAM] > 0 and deck >= 3):
            hand += 1
            deck -= 3

        # Rich Energy is Colorless but its attach is a guaranteed net +3 hand.
        if (not getattr(self.state, "energyAttached", False)
                and self.hand[C.ENRICHING_ENERGY] > 0 and deck >= 4):
            hand += 3
    ...
    def _boss_target_score(self, target):
        ...
        if self._active_win_reachable_without_gust() and not winning:
            # Never replace a deterministic game-winning Active KO with a
            # one-prize gust. A winning Bench target remains a legal alternative.
            _DIAG["terminal_boss_blocks"] += 1
            return -1
    ...
    def _score(self, o):
        ...
        if t == OptionType.END:
            ...
            attack_route = (self._meaningful_attack_offered()
                            or self._active_alakazam_can_be_fueled())
            return -1000 if attack_route else 0
        return 0
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.DUDUNSPARCE:
            ...
            if o.area != AreaType.BENCH and not any(
                    p is not None for p in self.me.bench):
                return -1
            if o.area == AreaType.BENCH and self._ko_and_continuity_locked():
                _DIAG["sufficient_damage_draw_blocks"] += 1
                return -1
    ...
    def _score_play_poke(self, card):
        ...
        if not self._grimmsnarl_bench_allowed(cid):
            _DIAG["grimmsnarl_bench_blocks"] += 1
            return -1
        emergency_body = 9000 if self.me.active and self._board_body_count() <= 1 else 0
        if cid == C.ABRA:
            ...
            if line >= 3:
                return 1500
            return 20000 - 250 * n + emergency_body
        if cid == C.DUNSPARCE:
            ...
            return 18500 - 250 * n + emergency_body
        if cid == C.FEZANDIPITI_EX:
            ...
            if not self._fez_bench_worthwhile():
                return -1
            if self._effect_breaker_required():
                return 22000 + emergency_body
            return (15200 if self._fez_alternate_matchup() else 12800) + emergency_body
        if cid == C.SHAYMIN:
            ...
            return (17000 + emergency_body
                    if (n == 0 and self._opp_threatens_bench()) else -1)
        if cid == C.PSYDUCK:
    ...
    def _active_win_reachable_without_gust(self):
        """A deterministic current-Active route already wins this turn."""
        active = self.me.active[0] if self.me.active else None
        target = self.opponent.active[0] if self.opponent.active else None
        if (active is None or target is None
                or prize_count(target) < len(self.me.prize)
                or self._attack_disabled_on_active()
                or not self._can_attack(active)):
            return False
        if self._active_best_dmg(target) >= target.hp:
            return True
        return bool(
            active.id == C.ALAKAZAM
            and not self._effect_prevented(target)
            and 20 * self._achievable_hand() >= target.hp
        )

    def _ko_and_continuity_locked(self):
        """Stop optional draw only for a terminal KO or an actual deck emergency.

        An ordinary midgame KO is not enough: spending hand builders there also
        develops the following turns and may expose a higher-value Boss target.
        """
        target = self.opponent.active[0] if self.opponent.active else None
        if (target is None or self._attack_disabled_on_active()
                or not self._lethal_now()):
            return False
        if prize_count(target) >= len(self.me.prize):
            return True
        return self._deck_preserve() and self._backup_eta() <= 1
    ...
    def _backup_eta(self):
        ...
        candy_route = self.hand[C.RARE_CANDY] > 0 and self.hand[C.ALAKAZAM] > 0
        step_route = self.hand[C.KADABRA] > 0 and self.hand[C.ALAKAZAM] > 0
        best = 99
        for pokemon in (self.me.bench or []):
            ...
            if pokemon.id in ALAKAZAM_IDS:
                ...
                fueled = self._can_attack(pokemon) or psychic_available
                if self.hand[C.ALAKAZAM] > 0:
                    best = min(best, 1 if fueled else 2)
                else:
                    best = min(best, 2 if fueled else 3)
            elif pokemon.id == C.ABRA:
                fueled = self._can_attack(pokemon) or psychic_available
                if candy_route:
                    best = min(best, 1 if fueled else 2)
                elif step_route:
                    evolution_wait = 2 if getattr(pokemon, "appearThisTurn", False) else 1
                    best = min(best, evolution_wait if fueled else evolution_wait + 1)
                elif self.hand[C.KADABRA] > 0:
                    best = min(best, 2 if fueled else 3)
                else:
                    best = min(best, 3)
        return best
    ...
    def _meaningful_attack_offered(self):
        target = self.opponent.active[0] if self.opponent.active else None
        if target is None or self._attack_disabled_on_active():
            return False
        return bool(
            self._active_best_dmg(target) > 0
            and any(option.type == OptionType.ATTACK
                    for option in (self.select.option or []))
        )
    ...
    def _score_play_trainer(self, card):
        ...
        if (cid in (C.HILDA, C.DAWN, C.POKE_PAD, C.BUDDY_POFFIN)
                and self._ko_and_continuity_locked()):
            _DIAG["sufficient_damage_draw_blocks"] += 1
            return -1
        ...
        if cid == C.BUDDY_POFFIN:
            ...
            return 24000 if self._board_body_count() <= 1 else 13000
        if cid == C.POKE_PAD:
        ...
        if cid == C.XEROSIC:
            ...
            mirror = any(p.id in (C.ABRA, C.KADABRA, C.ALAKAZAM, C.ALAKAZAM_PSY)
                         for p in opp_board)
            # Hand disruption is itself attack prevention in the mirror because
            # Powerful Hand's damage is 20 x hand size. Outside the mirror, wait
            # until it accompanies a real attack instead of spending a Supporter
            # on a setup-only turn.
            if (self._ko_and_continuity_locked()
                    or (not mirror and not self._meaningful_attack_offered())):
                return -1
        ...
        if cid == C.ENHANCED_HAMMER:
            attached = self._attached_special_energy_entries()
            meaningful = [
                entry for entry in attached
                if not self._hammer_target_is_doomed(entry[2], entry[1], entry[0])
            ]
            if not meaningful:
                return -1
            prevention = [
                entry for entry in meaningful if entry[2].id in EFFECT_PREVENT_ENERGY
            ]
            active = self.opponent.active[0] if self.opponent.active else None
            active_ko_after_spend = bool(
                active is not None
                and self._hammer_damage_after_spend(active) >= active.hp
            )
            if prevention:
                active_prevention = any(area == AreaType.ACTIVE for area, _, _ in prevention)
                if active_prevention:
                    # This is an attack unlock, not generic disruption.
                    return 26000 if self.me.handCount > 1 else 18000
                # A Bench Mist matters for the following turn, but only after the
                # current KO remains intact. Keep it just above the margin-KO attack.
                return 6500 if active_ko_after_spend else 15000
            ...
            if self._should_reserve_last_hammer():
                return -1
            # A normal tempo Hammer must not consume the hand card that supplies
            # exact Powerful Hand damage, nor hit the body already being KO'd.
            if self._lethal_now() and not active_ko_after_spend:
                return -1
            if meaningful:
    ...
    def _score_evolve(self, o):
        ...
        if o.inPlayArea == AreaType.BENCH and self._ko_and_continuity_locked():
            return 500
    ...
    def _enrich_draw_needed(self):
        opponent = self.opponent.active[0] if self.opponent.active else None
        required = math.ceil(getattr(opponent, "hp", 0) / 20) if opponent is not None else 0
    ...
    def _enriching_attach_score(self, pokemon, *, is_active=False):
        ...
        attack continuity in fixed-deck ablations, so v16 retains that gate
    ...
    def _fez_attach_score(self, pokemon, is_active, source=None):
        ...
        if mode == "ALTERNATE_ATTACKER":
            if self._effect_breaker_required() and attached < 3:
    ...
    def _score_attach(self, o):
        ...
        fez_in_play = self.field[C.FEZANDIPITI_EX] > 0
        if self._effect_breaker_required() and fez_in_play and p.id != C.FEZANDIPITI_EX:
            return -1
        ...
        if p.id == C.FEZANDIPITI_EX:
            return self._fez_attach_score(p, o.inPlayArea == AreaType.ACTIVE, src)
    ...
    def _score_attack(self, o):
        ...
        if aid == POWERFUL_HAND and self._effect_prevented(opp):
            # A zero-effect attack does not advance any prize or board race. End
            # instead when no Boss/Hammer/Fez route remains; otherwise those setup
            # actions already outrank this hard rejection.
            _DIAG["zero_effect_attacks_blocked"] += 1
            return -1
    ...
    def _score_attach_target(self, p, is_active):
        ...
        if (cc is not None and is_active and p.id in ALAKAZAM_IDS and self._should_fuel(p)
            ...
            return 28000
        if p.id == C.FEZANDIPITI_EX:
            return self._fez_attach_score(p, is_active, cc)
    ...
    def _abra_switch_score(self, card):
        """Post-Teleportation Attack pivot: preserve developed, fragile lines."""
        risk = self._opponent_can_ko_candidate(card)
        if card.id == C.DUDUNSPARCE:
            score = 1700
        elif card.id == C.DUNSPARCE:
            score = 1600
        elif card.id == C.FEZANDIPITI_EX:
            # A two-prizer is an acceptable early-game wall, not a late prize map.
            score = 1400 if len(self.opponent.prize) >= 5 else 100
        elif card.id in ALAKAZAM_IDS:
            score = 1250 if self._can_attack(card) else 1050
        elif card.id == C.KADABRA:
            score = 1100 if self.hand[C.ALAKAZAM] > 0 else 900
        elif card.id == C.ABRA:
            score = 1000
        else:
            score = 500
        if risk:
            # Losing Kadabra or an attacker before our next action destroys more
            # route value than feeding a spare Basic. Dunsparce remains the pivot.
            score -= 600 if card.id in (C.KADABRA, *ALAKAZAM_IDS) else 350
        return score
    ...
    def _score_active_choice(self, o, card):
        ...
        if o.playerIndex != self.my_index:
            return 0
        if (self.context == getattr(SelectContext, "SWITCH", object())
                and getattr(self._context_effect_card(), "id", None) == C.ABRA):
            return self._abra_switch_score(card)
    ...
    def _score_to_bench(self, card):
        ...
        if not self._grimmsnarl_bench_allowed(cid):
            _DIAG["grimmsnarl_bench_blocks"] += 1
            return -1
        emergency_body = 120 if self.me.active and self._board_body_count() <= 1 else 0
        if cid == C.ABRA:
            return 200 - 30 * n + emergency_body
        if cid == C.DUNSPARCE:
            return 140 - 30 * n + emergency_body
        if cid == C.FEZANDIPITI_EX:
...
def _remember_attack_disable(obs):
    ...
    if (_V9_STATE.get("attack_disabled_key") is not None
        ...
        # Retreating or evolving changes the in-play identity and clears the effect.
        _V9_STATE.update({"attack_disabled_key": None, "attack_disabled_turn": None})
