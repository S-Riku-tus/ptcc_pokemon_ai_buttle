...
# v16 retains only the two scopes that cleared accepted-action holdouts and a
# 2,000-game v11 gate: safe Abra/Dunsparce bench-role choice, and target choice
# for the same evolution card. Attack, Energy, Trainer, Boss, Hammer, and retreat
# remain deterministic.
ML_ALLOWED_ACTIONS = {"bench", "evolve"}
ML_SAFE_BENCH_IDS = {305, 741}  # Dunsparce, Abra
TACTICAL_RULE_ONLY_OPPONENT_IDS = {646, 647, 648}
FEZANDIPITI_EX = 140
SHAYMIN = 343
GENESECT = 142
...
def _option_context(
    ...
    return {
        ...
        "target_area": int(option.get("inPlayArea", option.get("area", -1)) or -1),
        "target_index": int(option.get("inPlayIndex", -1) or -1),
        "breaks_current_ko": bool(feature.get("breaks_current_ko_estimate", 0)),
        "attack_lethal": bool(feature.get("attack_lethal_estimate", 0)),
    }


def _semantic_key(context: dict[str, Any]) -> tuple[Any, ...]:
    """Collapse options that differ only by an interchangeable hand copy.

    The old bench-only live scope could select another copy of the same Abra or
    Dunsparce and report it as an ML override even though the game state after
    the action was identical.  Such copy swaps are diagnostics noise, not ML
    policy improvements.
    """
    return (
        context["action_type"],
...
def _fallback_scope_reason(context: dict[str, Any] | None) -> str | None:
    ...
    if action not in ML_ALLOWED_ACTIONS:
        return f"model_scope_excludes_{action}"
    return None
...
def _candidate_scope_reason(
    ...
    if fallback_context["action_type"] == "bench" and action == "bench":
        different_role = context["card_id"] != fallback_context["card_id"]
        both_safe = (
            context["card_id"] in ML_SAFE_BENCH_IDS
            and fallback_context["card_id"] in ML_SAFE_BENCH_IDS
        )
        if different_role and not both_safe:
...
class HybridRanker:
    ...
    def __init__(self, attacks: dict[int, dict[str, Any]] | None = None, threshold: float = 0.55):
        ...
        # This inherited safe scope passed its live gate and is on by default. Use the
        # versioned flag to disable it explicitly for deterministic A/B tests.
        override = os.environ.get("ALAKAZAM_ML_V16_ENABLE_OVERRIDE", "1")
        self.enable_override = override == "1"
        self.model_error = ""
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            **dict(self.diag),
            "runtime_scope": "live_guarded_bench_and_evolution_target_v16",
    ...
    def choose(self, observation: dict[str, Any], fallback_action: list[int]) -> list[int]:
        ...
        try:
            ...
            # Matchup safety rules (notably Grimmsnarl's multi-KO prize map) use
            # exact board composition that was not part of the distilled model.
            # Preserve the deterministic bench/evolution choice in those states.
            your_index = int(current.get("yourIndex", 0) or 0)
            players = current.get("players") or []
            opponent = players[1 - your_index] if len(players) == 2 else {}
            visible_ids = {
                int(card.get("id", -1))
                for card in ((opponent.get("active") or [])
                             + (opponent.get("bench") or [])
                             + (opponent.get("discard") or []))
                if isinstance(card, dict)
            }
            if visible_ids & TACTICAL_RULE_ONLY_OPPONENT_IDS:
                return self._fallback(fallback_action, "tactical_board_rule")
            ...
            fallback_key = _semantic_key(fallback_context)
            meaningful_safe = [
                index for index, is_safe in enumerate(safe)
                if is_safe and _semantic_key(contexts[index]) != fallback_key
            ]
            self.diag["semantic_equivalent_candidates"] += sum(
                int(is_safe and _semantic_key(contexts[index]) == fallback_key)
                for index, is_safe in enumerate(safe)
            )
            if not meaningful_safe:
                return self._fallback(fallback_action, "semantic_equivalent_only")
            ...
            class_threshold = float(thresholds.get(predicted_action, base_threshold))
            if predicted_action in ML_ALLOWED_ACTIONS:
                # Per-class thresholds were calibrated independently.  The old
                # global 0.55 floor made the promoted bench/evolve scope inert.
                probability_threshold = max(self.threshold_override, class_threshold)
            else:
                probability_threshold = max(base_threshold, class_threshold)
