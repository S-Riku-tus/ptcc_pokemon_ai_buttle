...
def test_v17_powerful_hand_into_mist_is_rejected_as_zero_progress():
    policy = h.load_policy()
...
def test_v18_boss_does_not_use_draw_routes_that_are_not_currently_legal():
    policy = h.load_policy()
    mine = h.Pokemon(policy.C.ALAKAZAM, hp=140, maxHp=140,
                     energies=[h.EnergyType.PSYCHIC])
    grimmsnarl = h.Pokemon(policy.GRIMMSNARL_EX_ID, hp=320, maxHp=320, playerIndex=1)
    ...
    # The bare state has no MAIN selection and therefore no offered Ability or
    # evolution action. v18 must not turn those hypothetical actions into a KO.
    assert obj._active_best_dmg(grimmsnarl) == 300
    assert obj._boss_active_reachable_damage(grimmsnarl) == 0
    assert obj._boss_target_score(morgrem) > 0
