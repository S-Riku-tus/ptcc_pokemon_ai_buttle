# alakazam_ml_v19 - current top-40 public-log imitation fixes:
#   [P0] Correct Nighttime Mine's card ID (1266, not Battle Cage 1264) and give it
#        a role gate so it cannot consume an attack/KO card as a generic Trainer.
#   [P1] Value Lana's Aid by its actual legal recovery count and net hand gain.
# Parent tactical core: v18.
...
class AlakazamPolicy:
    ...
    def _active_route_atoms(self):
        ...
        for option in (self.select.option or []):
            ...
            if card.id == C.ALAKAZAM:
                kind, delta, deck = "evolve_alakazam", 2, 3
            else:
                kind, delta, deck = "evolve_kadabra", 1, 2
            atoms.append({
                "kind": kind,
                "delta": delta,
                "deck": deck,
    ...
    def _active_route_plan(self):
        """Find the smallest deterministic legal route to KO the current Active.

        Unknown draws are never assumed. The route compares only actions that the
        engine currently offers: Hammer, evolution draws, Rich Energy,
        Dudunsparce, Dawn, and Hilda.
        """
        ...
        base_hand = self.me.handCount - int(hammer_ready)
        mandatory_actions = {"hammer"} if hammer_ready else set()
        immediate_damage = 20 * max(0, base_hand)
        target_prizes = prize_count(target)
        wins_on_ko = (
            target_prizes >= len(self.me.prize)
            or not any(pokemon is not None for pokemon in self.opponent.bench)
        )
        ...
        atoms = self._active_route_atoms()
        best = None
        for mask in range(1 << len(atoms)):
            ...
            key = (action_count, deck_cost, supporter_cost, -damage)
    ...
    def _active_route_action_score(self, kind):
        ...
        if not plan["ko"] or kind not in plan["actions"]:
            return -1
        if plan["winning"]:
            return 88000
    ...
    def _hand_delta(self, t, o):
        ...
        if t == OptionType.PLAY:
            card = get_card(self.obs, AreaType.HAND, o.index, self.my_index)
            if card is not None and card.id in (C.HILDA, C.DAWN):
                return 1
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.DUDUNSPARCE:
            ...
            if self._terminal_pivot_win(o):
                return 88000
            route_score = self._active_route_action_score("dudun")
            if route_score > 0:
                return route_score
    ...
    def _score_play_trainer(self, card):
        ...
        if route_kind is not None:
            route_score = self._active_route_action_score(route_kind)
            if route_score > 0:
                return route_score
    ...
    def _score_attach(self, o):
        ...
        if getattr(src, "id", None) == C.ENRICHING_ENERGY:
            route_score = self._active_route_action_score("enriching")
            if route_score > 0:
                return route_score
