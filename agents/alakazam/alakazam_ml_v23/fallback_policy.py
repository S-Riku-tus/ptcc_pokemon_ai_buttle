# alakazam_ml_v23 - v22 continuity policy + explicit opponent target control:
# Baseline is v22; v23 separates Boss target selection from own promotion/switching.
...
class AlakazamPolicy:
    ...
    def _boss_prize_upgrade_targets(self):
        """Immediate ex/Mega KOs that dominate an ordinary Active prize.

        Expert logs do not support blindly gusting every ex: removing Froslass,
        Alakazam, or another live engine can be worth more early. This gate is
        therefore limited to a low-role single-prize Active, a three-prize
        target, or a Boss KO that closes the game.
        """
        ...
        if active_ko and active_prizes >= len(self.me.prize):
            return []
        active_role = self._boss_role_bonus(active)
        targets = []
        for target in self.opponent.bench:
            ...
            if damage < max(1, int(getattr(target, "hp", 0) or 0)):
                continue
            closes_game = prizes >= len(self.me.prize)
            if not (
                    closes_game
                    or prizes >= 3
                    or active_role < 2200):
