...
def test_v23_keeps_live_single_prize_engine_exception_from_ex_gate():
    policy = h.load_policy()
    froslass = h.Pokemon(
        policy.FROSLASS_ID,
        hp=90,
    ...
    # Top teachers often remove a live Froslass/Alakazam engine rather than
    # blindly cashing an early ex. It can still be chosen by full target value,
    # but the new hard prize-upgrade gate must not erase that judgement.
