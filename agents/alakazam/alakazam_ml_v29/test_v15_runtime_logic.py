...
def test_v29_ml_scope_is_broad_residual_and_safety_guarded():
    ...
    assert 'ALAKAZAM_ML_V29_ENABLE_OVERRIDE", "1"' in source
    assert "fallback_selected" in source
    assert "legacy_ranker_score" in source
    assert "lethal_guard" in source
    assert "dudunsparce_body_floor" in source
    assert 'ALAKAZAM_ML_THRESHOLD", "0.20"' in main_source
