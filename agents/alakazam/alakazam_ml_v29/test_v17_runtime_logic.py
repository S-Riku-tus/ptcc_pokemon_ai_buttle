...
def test_v29_deck_is_preserved_and_legacy_ranker_is_byte_identical():
    import hashlib
    import json
    ...
    model = json.loads((here / "ranker_model.json").read_text(encoding="utf-8"))
    assert model["runtime_scope"] == "v29_residual_main_policy"
    assert model["training_decisions"] == 18336
    assert model["training_candidate_rows"] == 192179
