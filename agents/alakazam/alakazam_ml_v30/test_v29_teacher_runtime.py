"""Inherited safety regressions plus v30 model provenance."""
...
def test_v30_model_records_expanded_teacher_corpus():
    model = json.loads((HERE / "ranker_model.json").read_text("utf-8"))

    assert model["runtime_scope"] == (
        "v30_expanded_coherent_teacher_main_policy"
    )
    assert model["teacher_trajectories"] == 2_361
