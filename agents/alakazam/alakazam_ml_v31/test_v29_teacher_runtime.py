"""Inherited safety regressions plus v31 model provenance."""
...
def test_v31_model_records_majkel_ensemble():
    model = json.loads((HERE / "ranker_model.json").read_text("utf-8"))
    numeric = json.loads(
        (HERE / "ranker_numeric_model.json").read_text("utf-8")
    )
    ...
    assert numeric["ensemble_role"] == "numeric_id_diversifier"
    assert numeric["ensemble_weight"] == 1.3
    assert len(model["feature_names"]) > 600
    assert numeric["feature_names"] == model["feature_names"]
