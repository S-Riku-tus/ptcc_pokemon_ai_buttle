...
#   [P1] Treat a stranded support Active's Energy attach -> retreat -> attack as one route.
...
V27_SUPPORT_PIVOT_ENABLED = os.environ.get(
    "ALAKAZAM_V27_SUPPORT_PIVOT", "1"
) != "0"
V27_LINE_PRIORITY_ENABLED = os.environ.get(
    "ALAKAZAM_V27_LINE_PRIORITY", "1"
) != "0"
V27_DRAW_BUDGET_ENABLED = os.environ.get(
    "ALAKAZAM_V27_DRAW_BUDGET", "1"
) != "0"
pre_turn = -1
...
def _diag_template():
    return {
        ...
        "v22_continuity_draws": 0,
        "v22_satisfied_draw_blocks": 0,
        "v22_backup_prefuels": 0,
        ...
        # v26 board-count floor / matchup counters
        "v26_body_floor_plays": 0,
        "v26_body_floor_end_blocks": 0,
        "v26_dudun_body_floor_blocks": 0,
        "v26_hilda_dead_search_demotions": 0,
        "v26_dawn_body_searches": 0,
        "v26_checkup_engine_gusts": 0,
        # v27 target-aware hand budget / pivot / strategic-line counters
        "v27_support_pivot_attachments": 0,
        "v27_support_pivot_retreats": 0,
        "v27_exact_hand_draw_blocks": 0,
...
# v26: pre-evolution cardId -> the cards that evolve from it.  Used to see a
# bench-snipe attacker (Marnie's Grimmsnarl ex) one evolution BEFORE it comes
# online, so Flower Curtain can be set up in time instead of after the fact.
EVOLVES_FROM_INDEX = {}
for _c in all_card:
    _from = getattr(_c, 'evolvesFrom', None)
    if not _from:
        continue
    for _pre in all_card:
        if _pre.cardType == CardType.POKEMON and _pre.name == _from:
            EVOLVES_FROM_INDEX.setdefault(_pre.cardId, set()).add(_c.cardId)

# v26: abilities that drop damage counters on BOTH boards every Pokemon Checkup
# (Froslass' Freezing Shroud).  With two copies out this is 20 damage per turn on
# every one of our Ability Pokemon, which is what actually kills the Alakazam
# board in the Marnie's Grimmsnarl matchup — not the attacks.
CHECKUP_COUNTER_ABILITY_IDS = set()
for _c in all_card:
    for _s in (_c.skills or []):
        _t = (_s.text or '').replace('’', "'").lower()
        if ('checkup' in _t and 'damage counter' in _t
                and ('each pokemon' in _t or 'each pok' in _t)):
            CHECKUP_COUNTER_ABILITY_IDS.add(_c.cardId)
...
BOSS_KEY_ROLE_BONUS = {
    ...
    FROSLASS_ID: 4200,         # current top-five logs: selected in 19/25 appearances
    DURALUDON_ID: 4700,        # deny Archaludon ex plus its Energy acceleration
    ARCHALUDON_EX_ID: 4000,    # developed two-prize attacker / Energy accelerator
    678: 3200,                 # Mega Lucario ex
    CINDERACE_ID: 3200,       # Cinderace pressure attacker
    140: 3200,                 # current top-five logs: selected in 3/4 appearances
    112: 2800,                 # current top-five logs: most selected target (30/54)
    121: 3000,                 # Dragapult ex
    344: 2800,                 # Dwebble is the only route into the Crustle wall
    345: 3400,                 # Crustle (Mist matchup signature / wall)
    756: 4000,                 # Mega Kangaskhan ex: selected in 4/4 appearances
...
class AlakazamPolicy:
    ...
    def _opp_threatens_bench(self):
        """Would Flower Curtain actually save a body from bench damage?

        v26 widens two things that made the v24/v25 test fire far too late in the
        Marnie's Grimmsnarl matchup (0 wins in 3 live games, 8/27 pooled over
        v24+v25):

        * the snipe may still be one evolution away on their bench, and
        * a 30-point snipe kills an 80 HP Kadabra in three hits, not two.

        Both were misses in every recorded Grimmsnarl loss.  A live Froslass adds
        10 damage per checkup on top, so the effective window is shorter still.
        """
        damage = max(
            self._opponent_ready_bench_damage(), self._opponent_board_bench_damage()
        )
        if damage <= 0:
            return False
        hits = 4 if self._opp_has_froslass() else 3
        exposed = 0
    ...
    def _body_floor_critical(self):
        """One Knock Out away from losing on board count."""
        return self._board_body_count() <= 1

    def _offered_body_add(self):
        """A MAIN option that puts another Pokemon on our board right now."""
        if getattr(self, "context", None) != SelectContext.MAIN:
            return False
        if not self._open_bench():
            return False
        for option in (getattr(self.select, "option", None) or []):
            ...
            if (data.cardType == CardType.POKEMON
                ...
                return True
            if card.id == C.BUDDY_POFFIN:
                return True
        return False

    def _poffin_targets_left(self):
        """Basics still unseen that Buddy-Buddy Poffin can actually fetch (<=70 HP)."""
        total = 0
        for card_id, count in MY_DECK_COUNTS.items():
            data = card_table.get(card_id)
            if (data is None or data.cardType != CardType.POKEMON
                    or getattr(data, "stage1", False)
                    or getattr(data, "stage2", False)
                    or getattr(data, "hp", 0) > 70):
                continue
            total += max(0, count - self._visible_own_count(card_id))
        return total

    def _psychic_body_on_board(self):
        return any(
            pokemon is not None
            and getattr(card_table.get(pokemon.id), "pokemonType", None)
            == EnergyType.PSYCHIC
            for pokemon in self._my_board()
        )

    def _basic_search_targets_left(self, psychic_only=False):
        total = 0
        for card_id, count in MY_DECK_COUNTS.items():
            data = card_table.get(card_id)
            if (data is None or data.cardType != CardType.POKEMON
                    ...
                    or getattr(data, "stage2", False)):
                continue
            if psychic_only and getattr(data, "pokemonType", None) != EnergyType.PSYCHIC:
                continue
            total += max(0, count - self._visible_own_count(card_id))
        return total

    def _body_floor_play_score(self, card, attach=False):
        """Rank the ways to get off a one-body board; None when this play is not one.

        Order matters. Buddy-Buddy Poffin and a Telepath attach each put TWO
        bodies down at once, Poke Pad fetches a better body than Shaymin while
        keeping Flower Curtain in hand for the matchup that needs it, an
        Abra/Dunsparce is a real line body, Fezandipiti ex is a body that also
        hands over two prizes, and Shaymin is the last-resort body — still
        strictly better than boarding out, which is why it must beat END here.
        """
        cid = getattr(card, "id", None)
        data = card_table.get(cid)
        if data is None:
            return None
        if attach and cid != C.TELEPATH_ENERGY:
            return None
        if cid == C.BUDDY_POFFIN:
            return 62000 if self._poffin_targets_left() > 0 else None
        if cid == C.TELEPATH_ENERGY:
            if not self._psychic_body_on_board():
                return None
            if self._basic_search_targets_left(psychic_only=True) <= 0:
                return None
            return 61500
        if cid == C.POKE_PAD:
            return 60800 if self._basic_search_targets_left() > 0 else None
        if (data.cardType != CardType.POKEMON
                or getattr(data, "stage1", False)
                or getattr(data, "stage2", False)):
            return None
        ...
        if cid == C.FEZANDIPITI_EX:
            return 60500
        return 61000 - 100 * self.field[cid]

    def _evolution_target_on_board(self):
        """Is there a body a searched Evolution card could actually go onto?

        Hilda fetches an Evolution Pokemon; with no Abra/Kadabra/Dunsparce in
        play that card is dead this turn and the next, while Dawn's Basic keeps
        us on the board.  Live logs show the policy taking Hilda over Dawn at one
        body in eight games across v24/v25, five of them straight losses.
        """
        return any(
            pokemon is not None
            and pokemon.id in (C.ABRA, C.KADABRA, C.DUNSPARCE)
            for pokemon in self._my_board()
        )
    ...
    def _opponent_board_bench_damage(self):
        """Best bench-snipe damage anywhere on the opponent's board.

        v26: the v24/v25 test only looked at a payable attack on the opposing
        ACTIVE.  Against Marnie's Grimmsnarl ex the snipe comes online the turn
        Grimmsnarl evolves on the bench and is promoted, which is exactly one
        turn too late for Shaymin to matter.  Counting the whole board (and the
        evolutions of what is on it) turns Flower Curtain into a pre-emptive
        answer instead of a post-mortem one.
        """
        best = 0
        for pokemon in (self.opponent.active + self.opponent.bench):
            if pokemon is None:
                continue
            for card_id in self._evolution_closure(pokemon.id):
                data = card_table.get(card_id)
                if data is None:
                    continue
                for attack_id in (data.attacks or []):
                    if attack_id not in BENCH_DAMAGE_ATTACKS:
                        continue
                    best = max(
                        best, self._bench_damage_amount(ATTACK_TABLE.get(attack_id))
                    )
        return best

    def _evolution_closure(self, card_id):
        """card_id plus every card that evolves from it (one level deep, twice)."""
        seen = {card_id}
        frontier = [card_id]
        for _ in range(2):
            nxt = []
            for current in frontier:
                for candidate in EVOLVES_FROM_INDEX.get(current, ()):  # noqa: E501
    ...
    def _grim_ex_two_hit_pressure(self):
        ...
        if (
                active is None
                or active.id != GRIMMSNARL_EX_ID
                or prize_count(active) < 2
                or self._effect_prevented(active)):
            return False
        turn = getattr(self.state, "turn", None)
        if turn is not None and _V9_STATE.get("grim_pressure_turn") == turn:
            return True
    ...
    def _optional_draw_budget_met(self, kind):
        """The selected KO route does not need this optional hand-growth action.

        Powerful Hand needs exactly ``ceil(target.hp / 20)`` cards at attack
        time.  Once the highest-priority reachable target has a deterministic
        route that excludes this draw, activating Dudunsparce or Enriching only
        creates overkill and exposes more accumulated cards to hand reset.  The
        ability/ACE SPEC remains available for a later turn.
        """
        if not V27_DRAW_BUDGET_ENABLED or kind not in {"dudun", "enriching"}:
            return False
        plan = self._chosen_ko_plan()
        return bool(
            ...
            and plan["ko"]
            and plan["hand"] >= plan["required_hand"]
    ...
    def _draw_redundant_for_chosen_target(self, kind):
        ...
        plan = self._chosen_ko_plan()
        return bool(
            self._optional_draw_budget_met(kind)
            or (
                V20_HAND_GATE_ENABLED
                and plan is not None
                and plan["ko"]
                ...
                # Non-draw setup actions retain the inherited five-card buffer.
    ...
    def _evolution_descendants(self, card_id, max_depth=2):
        """Return visible card-database descendants without assuming an archetype.

        Target selection used to know only a short hand-written list of important
        IDs.  A Basic that becomes the opponent's central Stage 2/ex attacker was
        therefore treated like a disposable Basic until it evolved.  The card
        database already exposes the relationship, so carry the endpoint's role
        backward through at most the two legal evolution steps.
        """
        seen = set()
        frontier = {card_id}
        for _ in range(max(0, int(max_depth))):
            nxt = set()
            for current in frontier:
                for descendant in EVOLVES_FROM_INDEX.get(current, ()):
                    if descendant not in seen and descendant != card_id:
                        seen.add(descendant)
                        nxt.add(descendant)
            frontier = nxt
            if not frontier:
    ...
    @staticmethod
    def _strategic_endpoint(data):
        """A developed attacker/engine whose removal changes the opponent's plan."""
        if data is None:
            return False
        attacks = getattr(data, "attacks", None) or []
        skills = getattr(data, "skills", None) or []
        if not attacks and not skills:
            return False

        # ex/Mega and Stage 2 endpoints are normally the line's payoff.  For a
        # Stage 1, require either meaningful attack output or an engine Ability;
        # otherwise every incidental evolution line would be mislabeled as the
        # opponent's main strategy.
        if (
                getattr(data, "ex", False)
                or getattr(data, "megaEx", False)
                or getattr(data, "stage2", False)):
            return True
        if not getattr(data, "stage1", False):
            return False
        ...
        for attack_id in attacks:
            ...
            text = _norm_card_text(getattr(attack, "text", ""))
            if damage >= 100 or any(
                    marker in text
                    for marker in (
                        "for each", "times the number", "damage for every",
                        "more damage", "additional damage",
                    )):
                return True
        ...
        return any(
            marker in ability_text
            for marker in (
                "once during your turn", "pokemon checkup", "damage counter",
                "prevent all damage", "prevent all effects", "draw until",
            )
        )

    def _strategic_line_bonus(self, pokemon):
        """Role prior for a main endpoint and the Basics/Stage 1s leading to it.

        This is intentionally a role score, not a permission to ignore the prize
        race.  Terminal wins and multi-prize KOs remain the outer gates.  Within
        the same prize tier it establishes the natural order
        ``main attacker > Stage 1 > Basic > disposable body``.
        """
        if not V27_LINE_PRIORITY_ENABLED or pokemon is None:
            return 0
        data = card_table.get(pokemon.id)
        descendants = self._evolution_descendants(pokemon.id)
        meaningful_descendants = [
            card_table.get(card_id)
            for card_id in descendants
            if self._strategic_endpoint(card_table.get(card_id))
        ]
        if meaningful_descendants:
            if data is not None and getattr(data, "stage1", False):
                return 2300
            return 2300
        if not self._strategic_endpoint(data):
            return 0
        if getattr(data, "stage2", False) or getattr(data, "megaEx", False):
            return 3300
    ...
    def _boss_role_bonus(self, p):
        if p is None:
            return 0
        bonus = max(
            BOSS_KEY_ROLE_BONUS.get(p.id, 0),
            self._strategic_line_bonus(p),
        )
    ...
    def _checkup_counter_engine_upgrade(self, target, active):
        """Is gusting a Checkup damage-counter engine worth the same-prize trade?

        Froslass' Freezing Shroud puts a counter on every Ability Pokemon each
        Checkup.  Our board is Alakazam / Kadabra / Dudunsparce / Fezandipiti /
        Shaymin — all Ability Pokemon — so two copies erase 20 HP per body per
        turn.  In the three live Grimmsnarl games we never played a single Boss:
        the equal-prize value comparison always kept us on the Active, and an
        80 HP Kadabra died to pure Checkup chip in three turns.  Yushin Ito
        gusts Froslass 346 times in 1000 games.

        The exchange only qualifies as an upgrade when it costs us nothing in
        prizes and the engine is actually taxing at least two of our bodies.
        """
        if target is None or target.id not in CHECKUP_COUNTER_ABILITY_IDS:
            return False
        if active is not None and prize_count(target) < prize_count(active):
            return False
        taxed = sum(
            1
            for pokemon in self._my_board()
            if pokemon is not None
            and (card_table.get(pokemon.id) is not None)
            and (card_table[pokemon.id].skills or [])
        )
        if taxed < 2:
            return False
        _DIAG["v26_checkup_engine_gusts"] += 1
        return True
    ...
    def _boss_target_score(self, target):
        ...
        if active_ko:
            ...
            if not winning:
                role_upgrade = (
                    target_role >= active_role + 1500
                    or self._checkup_counter_engine_upgrade(target, active)
                )
    ...
    def _record_v9(self, selected):
        ...
        if self._boss_resolving():
            ...
            if (
                    ...
                    and self._strategic_line_bonus(target) > 0):
                _DIAG["v27_strategic_line_gusts"] += 1
            if (
                    isinstance(target, Pokemon)
        ...
        if self.context == getattr(SelectContext, "TO_ACTIVE", object()):
            ...
            if promoted is not None:
                key = ("v22_attack_route_promotions"
                       if self._promotion_attacks_next_turn(promoted)
                       else "v22_shield_promotions")
                _DIAG[key] += 1
            return
        if self.context != SelectContext.MAIN:
            return
        ...
        dudun_offered = any(
            candidate.type == OptionType.ABILITY
            and getattr(
                get_card(self.obs, candidate.area, candidate.index, self.my_index),
                "id",
                None,
            ) == C.DUDUNSPARCE
            and candidate.area == AreaType.BENCH
            for candidate in self.select.option
        )
        if (dudun_offered and self._dudun_draw_satisfied()
                and not self._continuity_draw_needed()):
            _DIAG["v22_satisfied_draw_blocks"] += 1
        if any(
                candidate.type == OptionType.PLAY
                and getattr(get_card(self.obs, AreaType.HAND, candidate.index, self.my_index), "id", None)
                == C.FEZANDIPITI_EX
        ...
        if option.type == OptionType.PLAY and getattr(card, "id", None) == C.FEZANDIPITI_EX:
            ...
            _DIAG["v22_dunsparce_rebuilds"] += 1
        elif option.type in (OptionType.ATTACH, OptionType.ENERGY):
            ...
            if target is not None and self._backup_prefuel_score(target, card) > 0:
                _DIAG["v22_backup_prefuels"] += 1
            ...
            if (
                ...
                elif self._support_pivot_attack_route(
                        target, option.inPlayArea, card) is not None:
                    _DIAG["v27_support_pivot_attachments"] += 1
            ...
            if active is not None and active.id in ONE_ENERGY_PIVOT_IDS:
                ...
                if self._support_pivot_attack_route(
                        active, AreaType.ACTIVE) is not None:
                    _DIAG["v27_support_pivot_retreats"] += 1
            if (
                    active is not None
    ...
    def _score(self, o):
        ...
        if (self.context == SelectContext.MAIN
            ...
            return -1
        # ── v26 P0: 盤面body下限 ────────────────────────────────────────────
        # Standing on a single body loses the game to one Knock Out. This has to
        # outrank the lethal-maintenance gate below: in 88457742 the gate scored
        # PLAY:Dunsparce at 10 to protect a 2-prize Powerful Hand, we attacked
        # with our only Pokemon, and boarded out on the reply. Every v25 game
        # that passed up a body at <=1 body was a loss (5/5).
        if (self.context == SelectContext.MAIN
                and t in (OptionType.PLAY, OptionType.ATTACH, OptionType.ENERGY)
                and self._body_floor_critical()
                and self._open_bench()):
            floor_score = self._body_floor_play_score(
                self._main_option_card(o), attach=(t != OptionType.PLAY)
            )
            if floor_score is not None:
                _DIAG["v26_body_floor_plays"] += 1
                return floor_score
        # ── v3 P0-1: 致死維持ゲート ──────────────────────────────────────────
        ...
        if t == OptionType.END:
            # A final safety net for the two ladder failure classes: never end a
            # turn while Alakazam can attack, or while one offered attachment can
            # make it attack. Normal setup still outranks the attack and runs first.
            opponent = self.opponent.active[0] if self.opponent.active else None
            meaningful_route = (
            ...
            if meaningful_route:
                return -1000
            # v26: never pass the turn on a single body while a Basic (or Poffin)
            # is still playable. Every v25 game that did this was lost.
            if self._body_floor_critical() and self._offered_body_add():
                _DIAG["v26_body_floor_end_blocks"] += 1
                return -2000
            return 0
        return 0
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.DUDUNSPARCE:
            ...
            if self._board_body_count() <= 1:
                return -1
            # v26: the v6 invariant only refused the cycle when Dudunsparce was
            # the LAST body, so a two-body board legally shrank to one and then
            # boarded out. 14 such cycles across v24/v25, 10 of them in losses
            # (88497435 t5, 88417447 t3, 88422743 t4). Cycling down to one body
            # is only allowed when the turn pays for it — a win, a same-turn KO
            # route, or a Basic/Poffin still on offer to put the body straight
            # back. It is refused for a bare draw or a reposition.
            body_floor_risk = (
                self._board_body_count() <= 2 and not self._offered_body_add()
            )
            if o.area != AreaType.BENCH and not any(
                    p is not None for p in self.me.bench):
                return -1
            low_deck_route = self._active_dudun_low_deck_cycle_route()
            if low_deck_route is not None:
            ...
            if route_score > 0:
                return route_score
            if body_floor_risk:
                _DIAG["v26_dudun_body_floor_blocks"] += 1
                return -1
    ...
    def _score_play_poke(self, card):
        ...
        if cid == C.DUNSPARCE:
            ...
            if self._opp_has_froslass() and engine >= 1:
                return 300    # do not expose a second draw-engine line to repeated counters
            if (len(self.opponent.prize) <= 3 and self._opp_threatens_bench()
                    and engine >= 1):
                return 300    # late spread phase: keep one engine, prune extra 70-HP prizes
            if engine >= 2:
                return 1200   # cap at 2 engine bodies
            line = (self.field[C.ABRA] + self.field[C.KADABRA]
                    + self.field[C.ALAKAZAM] + self.field[C.ALAKAZAM_PSY])
            if engine == 0 and line >= 2:
                # The top-eight logs rebuild the first cycle body before adding a
                # third/fourth redundant Abra.  This is the main attack-chain fix.
                return 20750
            return 18500 - 250 * n
        if cid == C.FEZANDIPITI_EX:
    ...
    def _backup_eta(self):
        ...
        for pokemon in (self.me.bench or []):
            ...
            if (pokemon.id == C.ABRA and self.hand[C.RARE_CANDY] > 0
                ...
                return 1
            if (pokemon.id == C.ABRA and self.hand[C.KADABRA] > 0
                    and self.hand[C.ALAKAZAM] > 0
                    and (self._energy_count(pokemon) > 0 or psychic_available)):
                # A normal Abra -> Kadabra -> Alakazam chain is deterministic,
                # but it still needs two future evolution turns.
                return 2
        return 99

    def _attack_chain_stable(self):
        """The current attack and a public one-turn replacement are both secured."""
        opponent = self.opponent.active[0] if self.opponent.active else None
        active = self.me.active[0] if self.me.active else None
        return bool(
            active is not None
            and active.id in ALAKAZAM_IDS
            and self._can_attack(active)
            and opponent is not None
            and not self._temporary_attack_immunity_applies(opponent)
            and not self._effect_prevented(opponent)
    ...
    def _backup_prefuel_score(self, pokemon, source):
        ...
        if (pokemon is None or pokemon.id not in (C.ABRA, C.KADABRA)
                or self._energy_count(pokemon) > 0
                or getattr(self.state, "energyAttached", False)
                or ENERGY_PROVIDES.get(getattr(source, "id", None)) != EnergyType.PSYCHIC):
            return -1
        active = self.me.active[0] if self.me.active else None
        opponent = self.opponent.active[0] if self.opponent.active else None
        if (active is None or active.id not in ALAKAZAM_IDS or not self._can_attack(active)
                or opponent is None or self._effect_prevented(opponent)
                or self._temporary_attack_immunity_applies(opponent)):
            return -1
        if any(
                other is not None and other is not pokemon
                and other.id in ALAKAZAM_IDS and self._can_attack(other)
                for other in self.me.bench):
            return -1
        if pokemon.id == C.KADABRA:
            deterministic = self.hand[C.ALAKAZAM] > 0
        else:
            deterministic = bool(
                self.hand[C.ALAKAZAM] > 0
                and (self.hand[C.RARE_CANDY] > 0 or self.hand[C.KADABRA] > 0)
            )
        if not deterministic:
            return -1
        # Above ordinary optional draw and below same-turn attacker fuel/Candy KO.
        return 18400 + (200 if pokemon.id == C.KADABRA else 0)
    ...
    def _active_dudun_low_deck_cycle_route(self):
        ...
        active = self.me.active[0] if self.me.active else None
        opponent = self.opponent.active[0] if self.opponent.active else None
        draws = min(3, max(0, self.me.deckCount))
        if (
                active is None
                or active.id != C.DUDUNSPARCE
                or opponent is None
                or draws not in (1, 2)
                or self._board_body_count() <= 1
                or self._effect_prevented(opponent)
        ...
        for attacker in self.me.bench:
            if attacker is None or attacker.id not in ALAKAZAM_IDS:
                continue
            route = self._candidate_attack_route(attacker, hand_adjust=draws)
            if route is not None and route["ko"]:
                routes.append(route)
        return max(routes, key=lambda route: route["score"]) if routes else None
    ...
    def _score_play_trainer(self, card):
        ...
        if cid in (C.HILDA, C.DAWN, C.POKE_PAD, C.BUDDY_POFFIN) \
            ...
            return -1
        # v26: Hilda fetches an Evolution Pokemon, Dawn fetches a Basic as well.
        # With no Abra/Kadabra/Dunsparce in play the Hilda card is dead on arrival,
        # and at one body it is the difference between rebuilding and boarding out.
        # v24/v25 logs: Hilda taken over Dawn at <=1 body in eight games, five lost.
        dead_evolution_search = not self._evolution_target_on_board()
        if cid == C.HILDA:
    ...
    def _dudun_retreat_attack_route(self, source=None):
        ...
        if (
                ...
                or self._temporary_attack_immunity_applies(opponent)):
            return None
        retreat_cost = self._route_retreat_cost(active)
        attached = self._energy_count(active)
        hand_adjust = 0
        if source is not None:
            if (
                    getattr(self.state, "energyAttached", False)
                    or getattr(source, "id", None) not in ENERGY_TYPES
                    or attached + 1 < retreat_cost):
                return None
            if source.id == C.ENRICHING_ENERGY:
                if not self._deck_spend_ok(cost=4, allow_lethal=True):
                    return None
                hand_adjust = self._guaranteed_hand_delta("enriching")
            else:
        ...
        for attacker in self.me.bench:
            if attacker is None or attacker.id not in ALAKAZAM_IDS:
                continue
            route = self._candidate_attack_route(
                attacker,
    ...
    def _support_pivot_attack_route(self, pokemon, area, source=None):
        """Evaluate support Active -> attach if needed -> retreat -> Alakazam attack.

        v26 had the pieces of this route, but Fezandipiti's card-specific attach
        branch returned before the generic pivot score was consulted.  Scoring
        the full route also accounts for the hand change: normal Energy is -1
        card, while Enriching Energy draws four for a net +3 before retreat.
        """
        if not V27_SUPPORT_PIVOT_ENABLED:
            return None
        active = self.me.active[0] if self.me.active else None
        opponent = self.opponent.active[0] if self.opponent.active else None
        if (
                pokemon is None
                or area != AreaType.ACTIVE
                or active is None
                or opponent is None
                or _pokemon_key(pokemon) != _pokemon_key(active)
                or pokemon.id not in ONE_ENERGY_PIVOT_IDS
                or not self._bench_attacker_ready()):
            return None

        # These cards are the verified one-Energy pivots.  Keep a floor of one
        # for compact/offline card stubs that omit retreatCost metadata.
        retreat_cost = max(1, self._route_retreat_cost(pokemon))
        shortfall = max(0, retreat_cost - self._energy_count(pokemon))
        hand_adjust = 0
        if source is not None:
            if (
                    getattr(self.state, "energyAttached", False)
                    or shortfall != 1
                    or not is_energy(getattr(source, "id", None))):
                return None
            if source.id == C.ENRICHING_ENERGY:
                if not self._deck_spend_ok(cost=4, allow_lethal=True):
                    return None
        ...
        for attacker in self.me.bench:
            ...
            if route is not None and route["damage"] > 0:
                routes.append(route)
        return max(routes, key=lambda route: route["score"]) if routes else None

    def _support_pivot_ready(self, pokemon, area, source=None):
        ...
        if (
                ...
                and pokemon.id == C.DUDUNSPARCE):
            return self._dudun_retreat_attack_route(source) is not None
        if source is None:
            return bool(
                pokemon is not None
                and area == AreaType.ACTIVE
        ...
        return self._support_pivot_attack_route(pokemon, area, source) is not None
    ...
    def _support_pivot_attach_score(self, pokemon, area, source=None):
        ...
        route = self._support_pivot_attack_route(pokemon, area, source)
        ...
        basic_bonus = 100 if getattr(source, "id", None) == C.PSYCHIC_ENERGY else 0
        return self._pivot_action_score(route) + basic_bonus
    ...
    def _score_attach(self, o):
        ...
        # v27: resolve the attach -> retreat -> attack route before card-specific
        # Fezandipiti/Rich branches can return.  Repositioning is the purpose of
        # this attachment even when no additional draw is needed.
        support_pivot_score = self._support_pivot_attach_score(
            p, o.inPlayArea, src
        )
        if support_pivot_score > 0:
            return support_pivot_score
        ...
        backup_score = self._backup_prefuel_score(p, src)
        if backup_score > 0:
            return backup_score
    ...
    def _score_retreat(self):
        ...
        if active is None or opp is None:
            return -1
        dudun_route = self._dudun_retreat_attack_route()
        if dudun_route is not None:
            return self._pivot_action_score(dudun_route)
        support_route = self._support_pivot_attack_route(
            active, AreaType.ACTIVE
        )
        if support_route is not None:
            return self._pivot_action_score(support_route)
    ...
    def _score_attack(self, o):
        ...
        if aid in (ABRA_TELEPORT, DUNSPARCE_TRADE):
            # These attacks end the turn. Switch only when the resulting Active
            # is a strictly better shield; do not expose an Alakazam-line asset
            # from behind an already-safe Dunsparce.
            shield, improvement = self._best_end_turn_switch_improvement(active)
            if shield is None:
                _DIAG["v28_end_turn_asset_blocks"] += 1
                return -1
            return min(5000, 1200 + improvement)
    ...
    def _score_card(self, o):
        ...
        if (
                ...
                and o.playerIndex == self.op_index):
    ...
    def _score_attach_target(self, p, is_active):
        ...
        support_pivot_score = self._support_pivot_attach_score(p, area, cc)
        if support_pivot_score > 0:
            return support_pivot_score
        ...
        backup_score = self._backup_prefuel_score(p, cc)
        if backup_score > 0:
            return backup_score
        if not self._should_fuel(p):
    ...
    def _opponent_can_ko_target_next_turn(self, target):
        """Conservative visible-board KO test used only for sacrifice choices."""
        opponent = self.opponent.active[0] if self.opponent.active else None
        data = card_table.get(opponent.id) if opponent is not None else None
        if target is None or opponent is None or data is None or not self._can_attack(opponent):
            return False
        base_damage = max(
        ...
        if base_damage >= getattr(target, "hp", 0):
            return True
        return bool(
            base_damage == 0
            and (getattr(data, "ex", False) or getattr(data, "megaEx", False))
            and self._energy_count(opponent) >= 2
        )

    def _promotion_attacks_next_turn(self, card):
        """Whether a post-KO promotion has a public, one-turn attack route."""
        if card is None:
            return False
        fueled = self._can_attack(card)
        can_attach = self._psychic_in_hand()
        if card.id in ALAKAZAM_IDS:
            return fueled or can_attach
        if card.id == C.KADABRA:
            return bool(self.hand[C.ALAKAZAM] and (fueled or can_attach))
        if card.id == C.ABRA:
            return bool(
                self.hand[C.RARE_CANDY] and self.hand[C.ALAKAZAM]
                and (self._energy_count(card) > 0 or can_attach)
            )
        if card.id == C.FEZANDIPITI_EX:
            return bool(self._can_attack(card) and self._fez_attack_goal())
        return False
    ...
    def _score_end_turn_switch_choice(self, card):
        """Abra/Dunsparce attacks end the turn: choose a shield, not an attacker."""
        if card.id == C.DUNSPARCE:
            return 9000
        if card.id == C.DUDUNSPARCE:
            return 8500
        if card.id == C.FEZANDIPITI_EX:
            ...
            return 7000 if safe_early else -2000
        # Preserve the evolution line unless every recyclable shield is absent.
        if card.id in ALAKAZAM_IDS:
            return 3200 + 300 * int(self._can_attack(card))
        if card.id == C.KADABRA:
            return 2200
        if card.id == C.ABRA:
            return 1200
        return 2800 + getattr(card, "hp", 0) // 20

    def _best_end_turn_switch_improvement(self, active):
        """Return the safest Bench body only when switching protects more value.

        Teleport and Trading Places end the turn immediately. Their MAIN score
        therefore has to use the same exposure ordering as the later switch
        sub-selection. Otherwise a merely legal switch scores above END and can
        put Abra/Kadabra/Alakazam in front of an already-safe Dunsparce.
        """
        if active is None:
            return None, 0
        active_score = self._score_end_turn_switch_choice(active)
        candidates = [
            pokemon for pokemon in self.me.bench
            if pokemon is not None
        ]
        if not candidates:
            return None, 0
        best = max(
            candidates,
            key=lambda pokemon: (
                self._score_end_turn_switch_choice(pokemon),
                -int(getattr(pokemon, "hp", 0) or 0),
                str(_pokemon_key(pokemon)),
            ),
        )
        improvement = self._score_end_turn_switch_choice(best) - active_score
        if improvement <= 0:
    ...
    def _score_active_choice(self, o, card):
        ...
        if (self.context == SelectContext.SWITCH
                and effect_id in (C.ABRA, C.DUNSPARCE)):
