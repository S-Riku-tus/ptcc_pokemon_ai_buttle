...
def _v35_safety_reason(
    ...
    *,
    ...
    """The v31 guards, with the two compulsions turned into prohibitions.

    ``v29_runtime._candidate_safety_reason`` is left untouched because the v29
    baseline's own choice feeds the ranker as a feature; changing it would move
    the corpus out from under the model. This is the ML path's copy, and it
    differs from the shared one in exactly two places.

    The lethal guard used to fire before the ranker was consulted at all,
    whenever the v29 baseline held an estimated-lethal Powerful Hand. That
    estimate means "KOs the opposing active", not "wins the game", so on the
    v34 holdout it fired on 1,122 of 9,977 decisions -- about six per game --
    and forced the attack immediately. The teacher usually banks value first:
    the ranker's pick matched the teacher on 79.7% of those decisions against
    52.6% for the forced attack, a net loss of 261 decisions. Powerful Hand
    deals 20 damage per card in hand, so the real risk in waiting is spending
    the hand down below lethal, and ``breaks_current_ko`` already refuses any
    action that does exactly that. What was missing is only the refusal to
    *throw the KO away*, so the guard now fires when the ranker would end the
    turn or swing with a non-lethal attack instead.

    The Boss guard used to hand the whole decision back whenever the v29
    baseline wanted Boss's Orders and the ranker did not, on the grounds that
    the fallback owns the nested target selection. It fired 326 times and the
    action it forced matched the teacher on 13.2% of them against 64.7% for
    the ranker, a net loss of 168 decisions. It now fires only when the ranker
    would end the turn, which is the one case where the route is genuinely
    stranded rather than deferred.

    Everything else -- unmodelled ``other`` actions, breaking the current KO,
    ending a turn with a ready Alakazam attack, and cycling the last
    Dudunsparce off a two-body board -- is unchanged.
    """
    ...
    if (
        baseline_context["action_type"] == "boss"
        and action == "end"
    ):
    ...
    if (
        action == "ability"
        and context["card_id"] == DUDUNSPARCE_CARD_ID
    ...
    if lethal_available:
        ...
        if action == "attack" and not context["attack_lethal"]:
...
def _flag(name: str, default: str) -> str:
    """v35 setting, with every earlier version's name still accepted."""
    for version in ("V35", "V34", "V33", "V32", "V31"):
        value = os.environ.get(f"ALAKAZAM_ML_{version}_{name}")
...
class HybridRanker:
    """v31 memory shell, narrowed v35 guards, and the v34 Yushin ranker."""
    ...
    def __init__(
        ...
        # "v35" narrows the lethal and Boss guards; "v34" restores the
        # deployed v31-v34 behaviour so the two shells can be raced against
        # each other with an identical model.
        self.shell = "v34" if _flag("SHELL", "v35") == "v34" else "v35"
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            **dict(self.diag),
            "runtime_scope": "v35_narrowed_shell_plus_recent_corpus_ranker",
            "shell": self.shell,
    ...
    def _safety_reason(
        ...
        *,
        ...
        """Dispatch to the deployed shell, or to the v34 shell for A/B runs.

        The v34 branch answers the same action as the deployed v34 agent on
        every decision, because every guard falls back to the same baseline
        pick and guard order therefore only changes the diagnostic label. It
        is not a latency replica: v34 vetoed a lethal board before scoring the
        ranker, and this branch scores first.
        """
        if self.shell == "v34":
            if lethal_available:
                return "lethal_guard"
            return _candidate_safety_reason(
                context,
                baseline_context,
                state,
    ...
    def choose(
        ...
        *,
        ...
        try:
            ...
            # v34 returned here. v35 carries the flag into the post-ranking
            # guard instead, so the ranker gets to answer the decision unless
            # its pick would actually throw the KO away.
            lethal_available = bool(baseline_context["attack_lethal"])
            if lethal_available:
                self.diag["lethal_available"] += 1
