...
def test_every_earlier_versions_env_names_still_reach_the_runtime():
    """v35 reads the flag chain in a loop; the old names must still work.

    This used to grep ml_runtime.py for each literal name. The loop that
    replaced the nested ``os.environ.get`` calls does not contain them, so the
    behaviour is asserted directly instead: newest name wins, and every
    earlier name is still honoured on its own.
    """
    import ml_runtime

    for version in ("V31", "V32", "V33", "V34", "V35"):
        variable = f"ALAKAZAM_ML_{version}_ENABLE_MEMORY"
        os.environ[variable] = "0"
        try:
            assert ml_runtime._flag("ENABLE_MEMORY", "1") == "0"
        finally:
            del os.environ[variable]
    assert ml_runtime._flag("ENABLE_MEMORY", "1") == "1"

    os.environ["ALAKAZAM_ML_V31_SHELL"] = "v34"
    os.environ["ALAKAZAM_ML_V35_SHELL"] = "v35"
    try:
        assert ml_runtime._flag("SHELL", "v35") == "v35"
    finally:
        del os.environ["ALAKAZAM_ML_V31_SHELL"]
