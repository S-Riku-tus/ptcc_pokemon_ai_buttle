...
class HybridRanker:
    """v31 safety shell with v34 ranker and v37 multi-teacher type ensemble."""
    ...
    def __init__(
        ...
        self.enable_override = os.environ.get(
            "ALAKAZAM_ML_V37_ENABLE_OVERRIDE",
            os.environ.get(
                "ALAKAZAM_ML_V36_ENABLE_OVERRIDE",
                os.environ.get(
                    "ALAKAZAM_ML_V35_ENABLE_OVERRIDE",
                    os.environ.get(
        ...
        self.enable_memory = os.environ.get(
            "ALAKAZAM_ML_V37_ENABLE_MEMORY",
            os.environ.get(
                "ALAKAZAM_ML_V36_ENABLE_MEMORY",
                os.environ.get(
                    "ALAKAZAM_ML_V35_ENABLE_MEMORY",
                    os.environ.get(
        ...
        self.rmy_type_model: dict[str, Any] | None = None
        ...
        try:
            ...
            # XGBoost evaluates features and split thresholds as float32.
            # Preserve that exact comparison contract in the pure-Python runtime.
            for tree in self.type_model.get("trees", []):
                tree["v"] = array("f", tree.get("v", []))
        except Exception as exc:
            self.errors["type_model"] = f"{type(exc).__name__}: {exc}"
        try:
            with open(_artifact_path("rmy_type_model.json"), encoding="utf-8") as handle:
                self.rmy_type_model = json.load(handle)
            for tree in self.rmy_type_model.get("trees", []):
                tree["v"] = array("f", tree.get("v", []))
        except Exception as exc:
            self.errors["rmy_type_model"] = f"{type(exc).__name__}: {exc}"
    ...
    def _type_meta_row(
        ...
        """Build the v36 rich action-type representation.

        The prefix is byte-for-byte compatible with v35's 356-value row.
        v36 then appends per-type route, lethal, hand-delta, draw, and option-
        ordering summaries.  Every value is computed from public runtime
        features and the already-deployed v34 candidate scores.
        """
        spec = self.type_spec or {}
        state_names = list(spec.get("state_names") or [])
        candidate_fields = list(spec.get("candidate_fields") or [])
        rich_top_fields = list(spec.get("rich_top_fields") or [])
        rich_agg_fields = list(spec.get("rich_agg_fields") or [])
        first = features[representatives[0]]
        row = [float(first.get(name, -1)) for name in state_names]
        type_peaks: list[float] = []
        type_locals: list[list[int]] = []
        type_tops: list[int | None] = []
        ...
        # v36 suffix: exact order matches research/rich_meta arrays.
        for action_type in range(12):
            local = type_locals[action_type]
            top = type_tops[action_type]
            if local and top is not None:
                top_feature = features[representatives[top]]
                row.extend(float(top_feature.get(name, -1)) for name in rich_top_fields)
                for name in rich_agg_fields:
                    values = [float(features[representatives[pos]].get(name, 0.0)) for pos in local]
                    row.extend([max(values), sum(values)])
                positions = [float(features[representatives[pos]].get("candidate_option_position", 0.0)) for pos in local]
                cards = {int(features[representatives[pos]].get("candidate_card_id", -1)) for pos in local}
                targets = {int(features[representatives[pos]].get("candidate_target_id", -1)) for pos in local}
                row.extend([
                    min(positions), sum(positions) / len(positions),
                    float(len(cards)), float(len(targets)),
                ])
            else:
                row.extend([-1.0] * len(rich_top_fields))
                row.extend([0.0] * (2 * len(rich_agg_fields) + 4))
        return row

    @staticmethod
    def _compact_type_probabilities(
        ...
        selected = array(
            "f",
            (float(row[int(index)]) for index in model.get("cols", [])),
        )
        ...
        return (
            [int(value) for value in model.get("classes", range(len(scores)))],
            [value / total for value in values],
        )

    def _predict_action_type(self, row: list[float]) -> tuple[int, float] | None:
        primary = self._compact_type_probabilities(self.type_model, row)
        if primary is None:
            return None
        classes, primary_probabilities = primary
        expert = self._compact_type_probabilities(self.rmy_type_model, row)
        probabilities = list(primary_probabilities)
        if expert is not None:
            expert_classes, expert_probabilities = expert
            if expert_classes == classes and len(expert_probabilities) == len(probabilities):
                weight = float((self.rmy_type_model or {}).get("weight", 0.15))
                weight = max(0.0, min(1.0, weight))
                probabilities = [
                    (1.0 - weight) * primary_value + weight * expert_value
                    for primary_value, expert_value in zip(
                        primary_probabilities, expert_probabilities
                    )
                ]
                self.diag["rmy_type_expert_blended"] += 1
    ...
    def snapshot(self) -> dict[str, Any]:
        ...
        return {
            **dict(self.diag),
            "runtime_scope": "v37_rmy_blended_type_gate_plus_v34_ranker_v31_safety",
            "override_enabled": self.enable_override,
            "memory_enabled": self.enable_memory,
            "model_loaded": self.model is not None,
            "rmy_type_model_loaded": self.rmy_type_model is not None,
    ...
    def choose(
        ...
        *,
        ...
        try:
            ...
            type_threshold = float((self.type_model or {}).get("threshold", 0.45))
