...
pb=np.load('/mnt/data/v36_work/rich_models/rich80.npz')['validation']
pe={n:np.load(f'/mnt/data/v37_work/experts/{n}.npz')['validation'] for n in ['yushin','majkel','rmy']}
yt=np.load('/mnt/data/v36_work/rich_meta/validation_y.npy')
names=json.load(open(D/'feature_names.json'));ai=names.index('action_type_id');X=np.load(D/'validation_features.npy',mmap_mode='r');lab=np.load(D/'validation_labels.npy',mmap_mode='r');g=np.load(D/'validation_groups.npy');bs=np.load(B/'v34_baseline_scores.npz')['validation']
...
json.dump([list(r) for r in res[:200]],open('/mnt/data/v37_work/expert_blend_validation.json','w'),indent=2)
np.savez_compressed('/mnt/data/v37_work/validation_policy_lookup.npz',H=H,HB=HB)
