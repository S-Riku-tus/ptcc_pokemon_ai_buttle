...
# v6 Boss targets. 675 is Team Rocket's Articuno in the current competition card
# table; it protects the Rocket board and is the concrete failure observed by the
# user.  The remaining IDs are high-impact engines/attackers seen repeatedly in
# the attached v5 ladder logs. Generic card-text/stage checks below remain the
# primary mechanism so this does not become a brittle matchup-only table.
ROCKET_ARTICUNO_ID = 675
BOSS_KEY_ROLE_BONUS = {
    ROCKET_ARTICUNO_ID: 7000,  # remove Rocket protection before attacking the core
    678: 3200,                 # Team Rocket's Spidops, main attacker
    666: 3200,                 # high-value two-prize attacker in Crustle/Cinderace shells
    140: 2600,                 # Fezandipiti ex draw engine / two-prize target
    112: 1800,                 # Munkidori damage-moving engine
    121: 3000,                 # Marnie's Grimmsnarl ex
    345: 3000,                 # Dragapult main evolution
    756: 3000,                 # Hop's Trevenant main attacker
    878: 2200,                 # Mega Lucario line
    879: 3000,
    647: 2200,                 # Festival lead attacker line
    648: 2800,
    C.ABRA: 900,
    C.KADABRA: 2600,
    C.ALAKAZAM: 3000,
...
class AlakazamPolicy:
    ...
    def _boss_target_score(self, target):
        """Return positive only when Boss creates a *better* same-turn KO.
        ...
        if damage <= 0 or damage < getattr(target, 'hp', 0):
            return -1
