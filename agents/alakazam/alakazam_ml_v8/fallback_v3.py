...
class AlakazamPolicy:
    ...
    def _boss_two_hit_target_score(self, target, damage):
        """Value a tightly gated two-attack route to a multi-prize Bench target.

        The opponent gets a turn to retreat or heal, so this is deliberately much
        narrower than the same-turn KO rule. It is allowed only when two Powerful
        Hand attacks cover the target, the prize race is urgent, and the target is
        sticky or strategically important enough to justify passing an Active KO.
        """
        ...
        prize_urgent = remaining <= target_prizes + 1
        major_upgrade = target_prizes >= 3 or (
            remaining <= 4 and target_prizes > active_prizes and role >= 2200
        )
        sticky_or_critical = (
            len(getattr(target, 'energies', []) or []) <= 1
            or role >= 3000
            or target_prizes >= 3
        )
        if not sticky_or_critical or not (prize_urgent or major_upgrade):
            return -1
    ...
    def _boss_target_score(self, target):
        ...
        if not active_ko and not (winning or two_prize or key_role or developed):
            return -1
    ...
    def _kadabra_target_bonus(self, o, target):
        """Prefer a Bench Kadabra when both Active and Bench Abra are legal.

        v7 selected the Active in all 35 observed dual-target ladder decisions due
        to a stable tie. An energy-ready Active Abra keeps priority only when its
        Kadabra attack takes an immediate KO.
        """
        both_areas = (
        ...
        if target_area == AreaType.ACTIVE:
            immediate_ko = bool(
                ...
                and self._energy_count(target) >= 1
                and self._alakazam_damage(SUPER_PSY_BOLT, opponent) >= opponent.hp
            )
            return 1400 if immediate_ko else 0
