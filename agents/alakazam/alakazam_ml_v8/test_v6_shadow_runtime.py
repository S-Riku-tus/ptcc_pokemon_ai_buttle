...
def test_live_override_is_opt_in(monkeypatch):
    ...
    assert runtime.snapshot()["runtime_scope"] == "shadow_guarded_bench_v8_rules"
...
def test_v8_specific_override_does_not_require_global_override(monkeypatch):
    monkeypatch.delenv("ALAKAZAM_ML_ENABLE_OVERRIDE", raising=False)
    monkeypatch.setenv("ALAKAZAM_ML_V8_ENABLE_OVERRIDE", "1")
    runtime = _runtime_module().HybridRanker()
    assert runtime.enable_override is True
