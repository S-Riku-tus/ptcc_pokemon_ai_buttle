# alakazam741_v6 - v5(sub54600504, ラダー結果)のユーザー所感に基づく改善。
#   [V6-1] 改造ハンマー: 使い所を限定。相手Activeのミスト/ロック等でフーディンの
#          Powerful Handが0にされていて、剥がせば攻撃が通り「このターン倒しきれる」
#          場合のみ使用(_hammer_unlocks_lethal, 16000)。ベンチ含む「特殊エネ見つけ次第
#          剥がす」9500分岐を廃止。例外として、使わないと相手の技でフーディン(自Active)が
#          KOされ負ける等の重大局面で、剥がせば相手が致死を出せなくなる時のみ阻止に使う
#          (_hammer_prevents_loss, 15000)。それ以外は -1。
#   [V6-2] クセロシキ: 既存トリガーに加え、(a)形勢不利かつ他の良いサポが無い/使わない
#          方が良く相手手札5枚以上、(b)クセロシキ未使用 or 手札2枚 かつ 相手手札6枚以上、
#          で発動可(_losing_position / _other_supporter_preferred)。
#   [V6-3] フーディン攻撃 vs のこっち退避: 退避(交換)判定の前に「攻撃すべきか」を判定。
#          ベンチに『次フーディンとして攻撃し相手Activeを倒せる』体があるなら、今の
#          Activeが次ターンKOされる見込みでも交換せず攻撃する(_saving_active_line を
#          _bench_attacker_can_ko でゲート)。ユーザー知見: 攻撃はいつかしないといけず、
#          居座られるとのこっちを狩られ続けて負けるため、継続手段がある限り殴る。
#   [V6-4] 山札切れ防止: ミストエネ2枚等『1枚のハンマーでは解除できない』詰みを
#          no_damage_path として掘りを停止(2枚以上のプレヴェント特殊エネ)。詰み中は
#          グッズ/サポの山札消費も停止(_deck_spend_ok / 個別ゲートに _no_damage_path)。
#          フロアに相手残サイド枚数も加味(_deck_floor)。
#   [V6-5] デッキ: のこっち(Dunsparce 305) 4→3 / ノココッチ(Dudunsparce 66) 3→2 /
#          メガディアンシーex(766) +1 / リーリエの決心(1227) 1→2。序盤「のこっち系しか
#          いない」事故と、盾/第2アタッカー不足の対策。766はたね・270HP・ダイヤコート
#          (被ダメ-30)・ギャランドレイ({P}{P}, 超エネ2枚捨てで最大240)。ただしメガex=
#          サイド3枚献上リスクがあるため盾運用は _shield_candidates の基準で制限。
# ---- v5(rev5.1)ヘッダ(経緯として保持) ----
...
#   実ログで確定した敗因と修正:
#   [P0-1] 最後の1体ノココッチ特性で即敗北が4件(85419845/85420867/85422380/85430151。
#          全て負け)。引き金は_item_locked()の状態誤検知(改造ハンマー等が「対象なしで
#          プレイ不可」なだけでロック扱い) → 前のRun Away Drawは「ベンチに攻撃可能
#          フーディンがいる」時だけ許可、盤面1体では-100000で絶対禁止。
#   [P0-2] 盤面全体の効果無効を検出できず0ダメPowerful Handを連打(85424359: ロケット団の
#          フリーザーの全体保護に19連発→山札切れ負け。85416758/85421379もミスト+ロック
#          付与相手に0ダメ連打、85421379は山札切れ負け)。特性テキストから全体保護持ちを
#          検出し、_effect_prevented()を盤面全体対応に。ボスの0ダメ対象呼び出しも自動で消える。
#   [P0-3] 0ダメしか出ない詰み対面(_no_damage_path)では任意ドロー(ノココッチ/ACTIVATE)を
#          停止し山札切れを防ぐ。サーチ(ヒルダ/ポケパッド等)は245/ユンゲラー到達手段として許可。
#   [P1-A] デッキ: ポケパッド 3→2 / リーリエの決心 0→1。ボスの指令は2枚維持(実ログ23回中
#          21回が次の自ターンまでにサイド獲得、23回全てで同ターン攻撃 → 「ボス0枚」仮説は
#          ログと矛盾)。クセロシキも3枚維持(2枚に減らすA/Bはミラー44%に悪化)。
#          フーディン245(ミスト/全体保護貫通)の採用は再A/Bでも却下: -1枚の743が響き
#          ミラー37%(600戦)。245の対応コードは温存(デッキに入れれば動く)。
#   [P1-B] リーリエの決心: 手札全戻し→6枚(自分サイド6なら8枚)ドロー。細い手札の立て直し
#          専用ロジック(使用順序: 使うべき札を使い切ってから/致死・完成コンボ保持時は禁止)。
#   [P2-A] 序盤盤面: 2回目の自ターン終了時に盤面5体≧の試合は勝率86.4%、4体以下は22.6%
#          (実ラダー53戦) → T4までは盤面5体未満ならポフィン最優先帯。
#   [P2-B] ミラーの昇格: 手札に進化フーディンが無い時はユンゲラー(80HP=相手PH圏)献上より
#          ノココッチ壁(140HP)/ダンスパ盾を優先(85449408: T9-11ユンゲラー放置→ライン崩壊)。
# ---- v4ヘッダ(経緯として保持) ----
# alakazam741_v4 - v3 + 51戦の実ラダーログ全数分析(sub54557078)に基づく改善。
#   分析手法: リプレイのaction↔observation対応を検証(応答はstep i+1に記録)し、全決定を
#   v3ポリシーで再評価。v3の「致死なのに攻撃せずEND」は0件・ボスのKO非直結発動も0件で
#   既存ゲートは機能済みと確認。真の敗因は:
#   [F1] アタッカー連続性の崩壊(最大): 負け21戦の非攻撃39ターン中34が「場にフーディン
#        不在」。被KO後にケーシィ/ユンゲラーが毎ターン狩られラインが完成しない
#        (vs メガルカリオ5-7 / エースバーン+アーケオスex 8-7)。
#   [F2] 序盤ベンチ切れ即負け4-5戦(T2-6でベンチ0のまま前を取られ敗北)。
#   [F3] ミスト付き相手へのPowerful Hand 0ダメ攻撃2回(85328678敗北)。
#   v4の変更:
#   [P0-A] ACE SPEC: リッチエネ(13)→ハイパーアロマ(1082, 進化1を3枚サーチ)。
#          ユンゲラー/ノココッチ供給=連続性の直接対策。リッチエネは{C}しか出せず超要求を
#          満たさず、ライン貼りで「エネはあるのに撃てない」事故もあったため撤去。
#   [P0-B] 連続性: ベンチ2体目フーディン進化を常時高優先(4000→18500)、前が攻撃可能に
#          なったらベンチのライン後続へ即エネ供給(1500→7600)、相手にex/メガがいる or
#          ライントラッシュ有なら4体目のライン展開解禁+ポフィンのボディ上限緩和。
#   [P0-C] 昇格改善: 手札にフーディンが無い時、80HPユンゲラーを献上せずノココッチ壁
#          (140HP)で1ターン稼ぎベンチのラインを守る。
#   [P0-D] 詰み解消: 前が攻撃不能&逃げエネ不足&ベンチに攻撃可能フーディン
#          → 前へエネを貼って逃げ道を作る(8800)→逃げる(6000)。
#   [P0-E] ベンチ切れガード: 盤面2体以下なら たね/ポフィン/夜のタンカ(たね回収)/
#          ヒカリ/ラナの助けを最優先帯へ。
#   [P1-F] ミスト対面: TO_HANDで改造ハンマーを優先取得。
# ---- v3ヘッダ(経緯として保持) ----
...
class C:
    ...
    MEGA_DIANCIE = 766    # v6 [V6-5] Basic Mega-ex: 270HP, Diamond Coat(被ダメ-30),
                          # Garland Ray {P}{P}: エネ2枚まで捨てて 120×捨てた枚数(最大240)。
                          # たね盾+第2アタッカー。ただしメガex=倒されるとサイド3枚。

    PSYCHIC_ENERGY = 5
    TELEPATH_ENERGY = 19  # special, provides {P}
    HYPER_AROMA = 1082    # ACE SPEC Item: search 3 Stage-1 (Kadabra/Dudunsparce) -> line throughput
    ENRICHING_ENERGY = 13 # (v4: デッキから撤去。サブ選択の互換のためIDのみ保持)

    BUDDY_POFFIN = 1086
    POKE_PAD = 1152
    HILDA = 1225          # Supporter: search Evolution + Energy
    LILLIE = 1227         # v5: Lillie's Determination — shuffle hand into deck,
...
GARLAND_RAY = 1110       # Mega Diancie ex 766: {P}{P} discard up to 2 energy, 120 x discarded (<=240)
...
# v5 [P0-2] BOARD-WIDE effect protection: some Pokémon protect OTHERS from attack
# effects while sitting anywhere in play (Team Rocket's Articuno 414: "Prevent all
# effects of attacks ... done to your Basic Team Rocket's Pokémon"). v4 only looked
# at the target itself and spent 19 turns doing 0 damage into a protected board
# (85424359, deck-out loss). Scan ability texts and build protector->predicate.
def _norm(s):
...
for _c in all_card:
    ...
    for _s in (_c.skills or []):
        ...
        if ('prevent all effects of attacks' in _t and 'done to your' in _t
            ...
            def _make_pred(scope):
                def pred(d):
                    if 'benched' in scope:
                        return False      # bench-only (e.g. Rabsca): the ACTIVE
                                          # target of our attacks is unaffected
                    if d is None:
                        return True       # unknown target -> be conservative
                    if 'basic' in scope and (getattr(d, 'stage1', 0) or getattr(d, 'stage2', 0)):
                        return False
                    if "team rocket" in scope and "team rocket" not in _norm(d.name):
                        return False
                    return True
                return pred
            GLOBAL_EFFECT_PROTECTORS[_c.cardId] = _make_pred(_scope)
...
class AlakazamPolicy:
    ...
    def _deck_preserve(self):
        ...
        if self._effect_prevented(opp):
            return False       # v5: 0ダメ対面の「勝っているつもり」で止まらない
    ...
    def _deck_floor(self):
        ...
        v6 [V6-4]: 勝ち切るのに要るターン数ぶんは山札を残す。自分の残サイド+バッファに
        加え、相手の残サイド枚数も下限に加味する(ユーザー要件: 引き続ける想定の保険)。"""
        return max(8, len(self.me.prize) + 3, len(self.opponent.prize))
    ...
    def _deck_spend_ok(self, cost=1, allow_lethal=True):
        ...
        v6 [V6-4]: 0ダメ詰み(no_damage_path)中は、山札を毎ターンの強制ドローだけに
        温存するため、グッズ/サポによるデッキ消費を一切許可しない。"""
        if self._no_damage_path():
            return False
    ...
    def _effect_prevented(self, target):
        ...
        for e in (getattr(target, 'energyCards', None) or []):
            if getattr(e, 'id', None) in EFFECT_PREVENT_ENERGY:
                return True
        # v5 [P0-2]: a protector anywhere on the opponent's board may cover the target
        d = card_table.get(target.id)
        for p in (self.opponent.active + self.opponent.bench):
            if p is not None and p.id in GLOBAL_EFFECT_PROTECTORS \
                    and GLOBAL_EFFECT_PROTECTORS[p.id](d):
                return True
        return False

    def _no_damage_path(self):
        """v5 [P0-3] 今の相手Activeに0ダメしか出せず、即座の解除手段も無い詰み状態。
        この間の任意ドローは山札切れ負けへの直行(85421379 / 85424359 の2敗が該当)。
        改造ハンマーで今剥がせる、または245フーディン(通常ダメージ)が場に居るなら
        通常運転を続ける。"""
        opp = self.opponent.active[0] if self.opponent.active else None
        if opp is None or not self._effect_prevented(opp):
            return False
        # 通常ダメージで貫通するアタッカー(245フーディン / メガディアンシーex)が場にいれば
        # 詰みではない(Psychic / Garland Ray は効果無効を無視してダメージが通る)。
        if any(p is not None and p.id in (C.ALAKAZAM_PSY, C.MEGA_DIANCIE)
               for p in self._my_board()):
            return False
        ...
        if self._opp_active_has_prevent_energy():
            pcount = self._opp_active_prevent_energy_count()
            # v6 [V6-4]: 1枚のハンマーで解除できる(=プレヴェント特殊エネがちょうど1枚)
            # 時だけ、ハンマーを探しに掘る。ミスト2枚等『1枚では通らない』時は掘っても
            # 山札切れに直行するだけなので、必要枚数のハンマーが手札に無い限り詰み扱い。
            if pcount <= 1:
                if self.hand[C.ENHANCED_HAMMER]:
                    return False
                hammers_seen = (self.hand[C.ENHANCED_HAMMER]
                                + self.discard.get(C.ENHANCED_HAMMER, 0))
                if hammers_seen < 4 and self.me.deckCount > self._deck_floor():
                    return False
            elif self.hand[C.ENHANCED_HAMMER] >= pcount:
                return False        # 2枚以上でも、必要数のハンマーが手札にあれば剥がして殴れる
        return True
    ...
    def _opp_active_prevent_energy_count(self):
        """相手Activeに付いている『効果を防ぐ特殊エネ(ミスト/ロック闘)』の枚数。
        改造ハンマー1枚で剥がせるのは1枚だけなので、この枚数が解除可否を決める。"""
        opp = self.opponent.active[0] if self.opponent.active else None
        if opp is None:
            return 0
        return sum(1 for e in (getattr(opp, 'energyCards', None) or [])
                   if getattr(e, 'id', None) in EFFECT_PREVENT_ENERGY)

    # -- v6 [V6-1] 改造ハンマーの使い所 --
    def _hammer_unlocks_lethal(self):
        """相手Activeのミスト/ロック等でフーディンのPowerful Handが0にされていて、
        1枚剥がせば攻撃が通り『このターン相手Activeを倒しきれる』か。"""
        opp = self.opponent.active[0] if self.opponent.active else None
        a = self.me.active[0] if self.me.active else None
        if opp is None or a is None:
            return False
        if a.id != C.ALAKAZAM or self._energy_count(a) < 1:
            return False
        d = card_table.get(opp.id)
        for p in (self.opponent.active + self.opponent.bench):
            if p is not None and p.id in GLOBAL_EFFECT_PROTECTORS \
                    and GLOBAL_EFFECT_PROTECTORS[p.id](d):
                return False        # 全体保護 -> 剥がしても通らない
        if opp.id in EFFECT_PREVENT_SELF:
            return False            # 自身特性の防止 -> ハンマー無効
        if self._opp_active_prevent_energy_count() != 1:
            return False            # 0枚(塞がれてない)/2枚以上(1枚では通らない)
        reach = 20 * max(0, self._achievable_hand() - 1)   # -1: 使うハンマー1枚ぶん
        return reach >= max(1, opp.hp)

    def _hammer_prevents_loss(self):
        """例外: 使わないと相手の技で自ActiveがこのままKOされ(=負け直結の重大局面)、
        かつ相手Activeの特殊エネを1枚剥がすと相手が致死を出せなくなる場合のみ使う。"""
        opp = self.opponent.active[0] if self.opponent.active else None
        a = self.me.active[0] if self.me.active else None
        if opp is None or a is None:
            return False
        has_special = any(
            (card_table.get(getattr(e, 'id', None)) is not None
             and card_table[e.id].cardType == CardType.SPECIAL_ENERGY)
            for e in (getattr(opp, 'energyCards', None) or []))
        if not has_special:
            return False
        if self._opp_max_damage_next_turn(a) < a.hp:
            return False            # そもそも次ターン致死されない -> 阻止不要
        lose_critical = (prize_count(a) >= len(self.opponent.prize)
                         or a.id in (C.ALAKAZAM, C.ALAKAZAM_PSY, C.MEGA_DIANCIE)
                         or self._board_bodies() <= 2)
        if not lose_critical:
            return False
        return not self._opp_can_ko_after_energy_loss(a, 1)

    def _opp_can_ko_after_energy_loss(self, target, remove=1):
        """相手Activeの付きエネを`remove`枚減らし1枚貼り直しても、`target`を致死できる
        攻撃が残るか(枚数コストベースの概算 -- テキスト加算は拾えない過小評価側)。"""
        opp = self.opponent.active[0] if self.opponent.active else None
        ...
        if oc is None:
            return False
        attached = len(opp.energies or [])
        avail = max(0, attached - remove) + 1
        td = card_table.get(target.id)
        for aid in (oc.attacks or []):
            if ATTACK_COST.get(aid, 0) > avail:
                continue
            ...
            if dmg <= 0:
                continue
            if td is not None and getattr(td, 'weakness', 0) \
                    and td.weakness in (opp.energies or []):
                dmg *= 2
            if dmg >= target.hp:
                return True
        return False

    # -- v6 [V6-2] クセロシキ発動の追加条件 --
    def _losing_position(self):
        """おおまかな形勢不利判定(厳密な判定は困難なため近似)。"""
        my_left = len(self.me.prize)
        opp_left = len(self.opponent.prize)
        if my_left > opp_left:
            return True
        if my_left == opp_left and not self._have_attacker() and self._opp_has_rulebox():
            return True
        return False

    def _other_supporter_preferred(self):
        """このターン、クセロシキより先に使いたい他サポが手札にあるか。"""
        if self.state.supporterPlayed:
            return False
        if (self.hand[C.HILDA] or self.hand[C.DAWN]) \
                and (self._need_pieces() or self._board_bodies() <= 2):
            return True
        if self.hand[C.LILLIE] and self.me.handCount <= 3 and not self._have_attacker():
            return True
        if self.hand[C.LANA_AID] and self._board_bodies() <= 2:
            return True
        return False

    # -- v6 [V6-3] フーディンで攻撃すべきか(退避の前段判定) --
    def _current_active_deals_damage(self):
        opp = self.opponent.active[0] if self.opponent.active else None
        return opp is not None and self._active_best_dmg(opp) > 0

    def _bench_attacker_can_ko(self):
        """ベンチに『次フーディンとして昇格すれば相手Activeを倒せる』後続がいるか。"""
        opp = self.opponent.active[0] if self.opponent.active else None
        if opp is None or self._effect_prevented(opp):
            return False
        for p in self.me.bench:
            if p is None or p.id not in ALAKAZAM_IDS or self._energy_count(p) < 1:
                continue
            if p.id == C.ALAKAZAM:
                if 20 * self._achievable_hand() >= opp.hp:
                    return True
            elif p.id == C.ALAKAZAM_PSY:
                if self._alakazam_damage(PSYCHIC_ATK, opp) >= opp.hp:
                    return True
        return False

    # -- v6 [V6-6] メガディアンシーex(766)への給エネ --
    def _diancie_attach_score(self, p):
        """Garland Ray({P}{P})が撃てるまで超エネを貼る(2枚まで=以降は過剰)。"""
        if self._can_pay(list(p.energies or []),
                         ATTACK_COST_ENERGIES.get(GARLAND_RAY,
                                                  [EnergyType.PSYCHIC, EnergyType.PSYCHIC])):
            return -1
        if self._effect_lock_matchup():
            return 8100
        return 2600 if self._opp_has_rulebox() else 1400
    ...
    def _saving_active_line(self):
        ...
        v6 [V6-3]: さらに、今のActiveがダメージを出せて、かつベンチに『次フーディンとして
        相手Activeを倒せる後続』がいるなら、交換せず攻撃する(継続手段があるので居座り
        負けを避ける)。この時Falseを返し、攻撃(_score_attack)がスコア序列で勝つ。"""
        a = self.me.active[0] if self.me.active else None
        if not (a is not None
            ...
            return False
        # 「フーディンで攻撃すべきか」の前段判定: 攻撃が通り、後続で継続できるなら攻撃。
        if self._current_active_deals_damage() and self._bench_attacker_can_ko():
            return False
        return True
    ...
    def _alakazam_damage(self, attack_id, target):
        ...
        elif attack_id == GARLAND_RAY:
            # v6 [V6-6] メガディアンシーex: 自分の超エネを2枚まで捨てて 120×捨てた枚数
            # (最大240)。ダメージ(効果ではない)なのでミスト/全体保護を貫通する。
            a = self.me.active[0] if self.me.active else None
            n = min(2, self._energy_count(a)) if (a is not None and a.id == C.MEGA_DIANCIE) else 0
            dmg = 120 * n
            od = card_table.get(target.id)
            if od is not None:
                if getattr(od, 'weakness', 0) == EnergyType.LIGHTNING:
                    dmg *= 2
                elif getattr(od, 'resistance', 0) == EnergyType.LIGHTNING:
                    dmg = max(0, dmg - 30)
            return dmg
        else:
            dmg = 0
    ...
    def _active_best_dmg(self, target):
        ...
        if self._energy_count(a) >= 1:
            ...
            if a.id == C.MEGA_DIANCIE:
                # v6 [V6-6]: 超2エネでGarland Ray(貫通、最大240)
                att = list(a.energies or [])
                if self._can_pay(att, ATTACK_COST_ENERGIES.get(
                        GARLAND_RAY, [EnergyType.PSYCHIC, EnergyType.PSYCHIC])):
                    return self._alakazam_damage(GARLAND_RAY, target)
        return 0
    ...
    def _hand_delta(self, t, o):
        ...
        if t == OptionType.PLAY:
            ...
            if card is not None and card.id == C.LILLIE:
                target = 8 if len(self.me.prize) == 6 else 6
                return target - self.me.handCount   # 全戻し→6/8枚
            return -1
        return 0
    ...
    def _score(self, o):
        ...
        if self.context == SelectContext.ACTIVATE and (
                self.me.deckCount <= self._deck_floor() or self._no_damage_path()):
            if t == OptionType.YES:
                return 0
            if t == OptionType.NO:
                return 1
    ...
    def _score_ability(self, o):
        ...
        if card.id == C.DUDUNSPARCE:
            ...
            if o.area != AreaType.BENCH:
                ...
                # v5 [P0-1] 絶対禁止: 場が1体の時に使うと場が0体になり即敗北。
                # 実ラダー4敗(85419845 T3 / 85420867 T7 / 85422380 T4 / 85430151 T10)は
                # 全て「_item_locked()の誤検知(手札のグッズが対象なしでプレイ不可な
                # だけ)→ベンチ0でRun Away Draw」だった。ロック脱出目的の起動は
                # どのみち昇格先が要るので、ベンチの攻撃可能フーディンがいる時だけ許可。
                if self._board_bodies() <= 1:
                    return -100000
                if self._bench_attacker_ready():
                    return 14000
                return -1
            ...
            if self._deck_preserve():
                return -1
            # v5 [P0-3]: 0ダメ詰み対面では3枚ミル(自身も山札へ)を止める
            if self._no_damage_path():
                return -1
            # v5 [P0-1]: 盤面2体(前+この1体)で使うと場が1体になる。手札から補充
            # できない状態なら、次の前KOで即敗北圏に入るため止める。
            if self._board_bodies() <= 2 and not (
                    self.hand[C.ABRA] or self.hand[C.DUNSPARCE]
                    or (self.hand[C.BUDDY_POFFIN] and self._open_bench())):
                return -1
    ...
    def _score_play_poke(self, card):
        ...
        if cid == C.ABRA:
            ...
            # v4 [P0-B]: …ただし実ラダーの負けは「ライン枯渇」34ターンが最大要因。
            # 相手がex/メガ体制 or ラインが既に墓地へ落ちている(=毎ターン狩られる展開)
            # なら4体目まで解禁して補充を続ける。ベンチ切れ間際(盤面<=2)は無条件で出す。
            line = (self.field[C.ABRA] + self.field[C.KADABRA]
                    + self.field[C.ALAKAZAM] + self.field[C.ALAKAZAM_PSY])
            if self._board_bodies() <= 2:
                return 22000
            cap = 4 if (self._opp_has_rulebox() or self._line_in_discard() >= 1) else 3
            if line >= cap:
                return 1500
            return 20000 - 250 * n
        if cid == C.DUNSPARCE:
        ...
        if cid == C.MEGA_DIANCIE:
            # v6 [V6-6]: 270HPたね盾+第2アタッカー。空きベンチがあれば出しておく
            # (前に出すか否かは _score_active_choice でサイド3枚を踏まえ慎重に判断)。
            return -1 if n >= 1 else 14500
        return 14000 - 200 * n
    ...
    def _board_bodies(self):
        """v4 [P0-E]: 自分の場のポケモン総数。<=2はベンチ切れ即負け圏(実ログ4-5敗)。"""
        return sum(1 for p in (self.me.active + self.me.bench) if p is not None)

    def _opp_has_rulebox(self):
        """相手の場にex/メガ(=こちらの80HP以下を毎ターン一撃してくる圧)がいるか。"""
        for p in (self.opponent.active + self.opponent.bench):
            d = card_table.get(p.id) if p is not None else None
            if d is not None and (d.ex or d.megaEx):
                return True
        return False

    def _opp_is_mirror(self):
        """相手もフーディンライン(=Powerful Handで80HPを一撃できる)か。"""
        return any(p is not None and p.id in (C.ABRA, C.KADABRA, C.ALAKAZAM, C.ALAKAZAM_PSY)
                   for p in (self.opponent.active + self.opponent.bench))

    def _line_in_discard(self):
        return sum(self.discard.get(x, 0) for x in
    ...
    def _score_play_trainer(self, card):
        ...
        if cid == C.BUDDY_POFFIN:
            if not self._open_bench():
                return 600
            # v4 [P0-E]: 盤面2体以下=次の前KOで即負け圏。最優先で体を足す(実ログ4-5敗)。
            if self._board_bodies() <= 2:
                return 16000
            # v5 [P2-A]: 2回目の自ターン終了時 盤面5体≧ → 勝率86.4% / 4体以下22.6%
            # (v4実ラダー53戦)。序盤(T4まで)は5体到達を明示目標にする。
            if self.state.turn <= 4 and self._board_bodies() < 5:
                return 15200
            ...
            # v4 [P0-B]: ライン枯渇圧(相手ex/メガ or ライン墓地落ち)の間は補充を続ける
            if bodies >= 4 and not (self._opp_has_rulebox() or self._line_in_discard() >= 1):
        ...
        if cid == C.LILLIE:
            # v5 [P1-B] リーリエの決心: 手札を山札に全て戻し6枚(自分サイドが
            # ちょうど6枚なら8枚)ドロー。用途は(1)細い手札/クセロシキ被弾後の立て直し
            # (2)山札回復(手札-1枚ぶんは山に戻る)。
            # 使用順序は素点で制御: たね(20000+)/進化(18500+)/エネ貼り/ハンマー(16000)
            # など「今使うべき札」より必ず下 → それらを使い切ってから最後に切れる。
            # ポケパッド等のサーチ札はリーリエの後に回す(先に使うと検索結果ごと戻す)。
            if self.state.supporterPlayed:
                return -1
            target = 8 if len(self.me.prize) == 6 else 6
            eff = self.me.handCount - 1                # リーリエ自身を除く
            if self._lethal_attack_offered():
                return -1                              # 今の手札で倒せるなら崩さない
            keeps_combo = (
                (self.hand[C.ALAKAZAM] or self.hand[C.ALAKAZAM_PSY])
                and (self.hand[C.KADABRA] or self.field[C.KADABRA]
                     or (self.hand[C.RARE_CANDY] and self.field[C.ABRA])))
            if keeps_combo and eff >= 3:
                return -1                              # 完成コンボは手放さない
            if eff <= target - 3 and not self._have_attacker():
                return 14200      # 事故立て直し: ヒルダ(12500)/ヒカリ(12000)より優先
            if eff <= target - 2:
        ...
        if cid == C.XEROSIC:
            ...
            if opp_hand >= 8:
                return 13000
            # v6 [V6-2b]: クセロシキ未使用 or 手札に2枚 かつ 相手手札6枚以上なら使って良い。
            if (self.discard.get(C.XEROSIC, 0) == 0 or self.hand[C.XEROSIC] >= 2) \
                    and opp_hand >= 6:
                return 12000
            if mirror and opp_hand >= 4:
                return 9000
            # v6 [V6-2a]: 形勢不利で、他に使うべきサポが無い/使わない方が良く、
            # 相手手札5枚以上なら妨害に使う。
            if self._losing_position() and opp_hand >= 5 \
                    and not self._other_supporter_preferred():
                return 8700
            return 400              # 手札が細い相手には温存
        if cid == C.ENHANCED_HAMMER:
            # v6 [V6-1]: 使い所を限定。基本は「相手ActiveのミストなどでPowerful Handが
            # 0にされていて、剥がせばこのターン倒しきれる」時だけ使う(_hammer_unlocks_lethal)。
            # 「特殊エネを見つけ次第剥がす(ベンチ含む)」旧9500分岐は廃止。
            if self._hammer_unlocks_lethal():
                return 16000
            # 例外: 使わないと相手の技で自ActiveがKOされ負ける等の重大局面で、
            # 剥がせば相手が致死を出せなくなる時のみ阻止に使う。
            if self._hammer_prevents_loss():
                return 15000
            return -1
        ...
        if cid == C.NIGHT_STRETCHER:
            basic_in_discard = self.discard.get(C.ABRA, 0) or self.discard.get(C.DUNSPARCE, 0)
            # v4 [P0-E]: ベンチ切れ間際で墓地にたねが居るなら回収が最優先帯。
            if self._board_bodies() <= 2 and basic_in_discard and self._open_bench():
                return 15500
        ...
        if cid == C.LANA_AID:
            if self.state.supporterPlayed:
                return -1
            # v4 [P0-E]: ベンチ切れ間際なら墓地から体を拾う(たね回収=延命)。
            if self._board_bodies() <= 2 and any(
                    self.discard.get(x, 0) for x in (C.ABRA, C.DUNSPARCE)):
                return 13000
            return 6000 if self._low_deck() else 1500
        if cid == C.HYPER_AROMA:
            # v4 [P0-A] ACE SPEC: 進化1(ユンゲラー/ノココッチ)を3枚サーチ。
            # ライン連続性(最大敗因)への直接回答。ユンゲラー在庫が薄い/ライン再建中に
            # 高優先で切る。デッキにユンゲラーが残っていなければ温存。
            kad_seen = self.hand[C.KADABRA] + self.field[C.KADABRA] + self.discard.get(C.KADABRA, 0)
            kad_in_deck = 4 - kad_seen - sum(1 for c in self.me.prize if getattr(c, 'id', None) == C.KADABRA)
            if kad_in_deck <= 0:
                return 700
            if not self._deck_spend_ok(cost=2, allow_lethal=False):
                return -1
            rebuilding = (self.field[C.ALAKAZAM] + self.field[C.ALAKAZAM_PSY] <= 1
                          or self._line_in_discard() >= 1)
            if self.hand[C.KADABRA] == 0 and rebuilding:
                return 15800
            return 9000 if rebuilding else 3000
        if cid == C.SACRED_ASH:
    ...
    def _score_evolve(self, o):
        ...
        if cid == C.ALAKAZAM_PSY:
            ...
            return 20400
        if cid == C.ALAKAZAM:
            # v4 [P0-B]: 実ラダーの最大敗因は「前のフーディンが死んだ次のターンに
            # 後続が居ない」(非攻撃39ターン中34)。手札1枚(-20ダメ)より2体目の常備が
            # 圧倒的に価値が高い → ベンチのユンゲラー進化も常時高優先。3体目以降のみ抑制。
    ...
    def _score_attach(self, o):
        ...
        # v6 [V6-6]: メガディアンシーexはGarland Rayが{P}{P} — 超以外(エンリッチ等)は
        # 貼らず、専用ロジックで2エネまで供給する。
        if p.id == C.MEGA_DIANCIE:
            if src is not None and ENERGY_PROVIDES.get(src.id) not in (EnergyType.PSYCHIC, None):
                return -1
            return self._diancie_attach_score(p)
        ...
        if p.id in ALAKAZAM_IDS:
            ...
            # v4 [P0-B]: 前のアタッカーが既に攻撃可能なら、次のターンに前が倒された時
            # そのまま昇格→Powerful Handできるよう後続へ先貼りする(連続性>手札+20ダメ)。
            base = 7600 if self._have_attacker() else 1500
        else:
            return -1             # non-attacker -> don't waste energy, hold it
        # v4 [P0-D] 詰み解消: 前が攻撃も逃げも出来ず、ベンチに攻撃可能フーディンが
        # 居るなら、前へ貼って逃げエネを確保する(この後 _score_retreat 6000 で逃げる)。
        a = self.me.active[0] if self.me.active else None
        if (a is not None and o.inPlayArea == AreaType.ACTIVE and p.id == a.id
                and not self._can_attack(a) and self._bench_attacker_ready()):
            d = card_table.get(a.id)
            rc = getattr(d, 'retreatCost', 0) if d is not None else 0
            if len(a.energies) < rc:
                base = max(base, 8800)
    ...
    def _score_attach_target(self, p, is_active):
        ...
        if p.id == C.MEGA_DIANCIE:
            return self._diancie_attach_score(p)    # v6 [V6-6]: Garland Ray 超2エネまで
        if not self._should_fuel(p):
            return -1             # already CAN attack (type-aware) -> don't over-fill
        if p.id in ALAKAZAM_IDS:
            return 8000 + (200 if is_active else 0)
        if p.id in (C.ABRA, C.KADABRA):
            if is_active and self._bench_attacker_ready():
                return 8800       # v4 [P0-D]: 逃げエネ確保(詰み解消)
            return 7600 if self._have_attacker() else 1500
        return -1
    ...
    def _score_active_choice(self, o, card):
        ...
        if card.id in ALAKAZAM_IDS:
            score += 300         # a powered Alakazam = our attacker
        elif card.id == C.KADABRA:
            # v4 [P0-C]: 手札にフーディンがあれば昇格→即進化→Powerful Handが最善。
            # 無ければ80HPのユンゲラーはex/メガの一撃で狩られてライン枯渇が進むだけ
            # → ノココッチ壁より下げて後続を守る(実ログ: ユンゲラー献上→枯渇が頻発)。
            if self.hand[C.ALAKAZAM] or self.hand[C.ALAKAZAM_PSY]:
                score += 180
            elif self._opp_has_rulebox():
                score += 55
            elif self._opp_is_mirror():
                # v5 [P2-B]: ミラーでは80HPユンゲラー=相手PH一撃圏。進化できないなら
                # 献上せずダンスパ系の後ろに置く(85449408: T9-11放置→ライン崩壊)。
                score += 60
            else:
                score += 95
        elif card.id == C.ABRA:
            ...
            # v4 [P0-C]: 相手がex/メガ体制でラインの後続(進化待ち)をベンチに抱えて
            # いる時は、140HP壁で1ターン買うのが正解(ユンゲラー即進化できる時を除く)。
            line_waiting = any(p is not None and p.id in (C.ABRA, C.KADABRA)
                               for p in self.me.bench)
            if (self._opp_has_rulebox() or self._opp_is_mirror()) and line_waiting \
                    and not (self.hand[C.ALAKAZAM] or self.hand[C.ALAKAZAM_PSY]):
                score += 120
            else:
        ...
        elif card.id == C.MEGA_DIANCIE:
            # v6 [V6-6]: 270HP+ダイヤコートの非常に硬い壁。攻撃も可能(Garland Ray)。
            # ただしメガex=倒されるとサイド3枚 → それで相手が勝ち切る状況では前に出さない。
            if prize_count(card) >= len(self.opponent.prize):
                score -= 120
            elif self._can_attack(card):
                score += 260     # 給エネ済み=攻撃できる硬い壁(フーディン+300よりは下)
            else:
                score += 140     # 硬い壁として昇格(裸ケーシィ/ユンゲラー献上よりは上)
    ...
    def _score_setup_active(self, card):
        ...
        if card.id == C.MEGA_DIANCIE:
            # v6: 270HP+ダイヤコートの硬い壁で序盤を凌げるが、起動が遅くサイド3枚。
            # ケーシィ/のこっちの次点(techよりは上)。
            return 20
    ...
    def _score_to_bench(self, card):
        ...
        if cid == C.MEGA_DIANCIE:
            return 130 - 20 * n   # v6: 270HP壁+第2アタッカー
        return 100 - 20 * n
    ...
    def _score_to_hand(self, card):
        ...
        # v4 [P0-E]: 盤面が薄い時はたねを最優先で確保(ベンチ切れ負け対策)。
        if self._board_bodies() <= 2 and cid in (C.ABRA, C.DUNSPARCE):
            score += 220
        if cid == C.DUDUNSPARCE:
        ...
        elif cid == C.MEGA_DIANCIE:
            # v6: 効果無効対面ではダメージ貫通の第2アタッカーとして最優先確保。
            score += 260 if self._no_damage_path() else 40
        elif cid == C.HYPER_AROMA:
            score += 70   # ACE SPEC — ライン供給(v4)
        elif cid == C.ENHANCED_HAMMER:
            # v4 [P1-F]: ミスト/ストーン闘系が見えている相手には改造ハンマーを最優先確保
            if any(getattr(e, 'id', None) in EFFECT_PREVENT_ENERGY
    ...
    def _score_discard(self, card):
        ...
        if cid in (C.ABRA, C.KADABRA, C.ALAKAZAM, C.DUNSPARCE, C.DUDUNSPARCE,
                   C.DUDUNSPARCE_EX, C.MEGA_DIANCIE):   # 306/766は1枚しか無い — 捨てない
    ...
    def _score_putback(self, card):
        ...
        if self.hand[cid] >= 2:
            return 70
        if cid in (C.ABRA, C.KADABRA, C.ALAKAZAM, C.DUNSPARCE, C.DUDUNSPARCE,
                   C.DUDUNSPARCE_EX, C.MEGA_DIANCIE):   # 306/766(各1枚)は場に無い限り手放さない
