...
2. `PRE_ATTACK`: 安全なRun Away Draw、最初の後続確保、必要なスタジアム張り替え、ミラーで相手手札6枚以上のクセロシキ
3. `ATTACK`: Powerful Handによる攻撃
4. `BUILD_ATTACKER`: 今ターンの攻撃役を作る進化・給エネ
5. `BUILD_BACKUP`: 後続の準備
6. `SEARCH`: 必要札のサーチ・立て直し
7. `DISRUPT`: 通常時のクセロシキ、改造ハンマー、通常時のスタジアム
8. `END`: ターン終了
...
カード個別評価では、フーディンミラーで相手手札が6枚以上のクセロシキを最優先としていましたが、従来はTier側で一律`DISRUPT`に分類され、通常攻撃より後ろへ落ちていました。

この版では、`PRESSURE`または`ENDGAME`かつ相手盤面がフーディンラインで、相手手札が6枚以上の場合だけ`PRE_ATTACK`へ昇格します。相手手札5枚以下、非ミラー、現在の確定KOを崩す場合は従来どおり攻撃を優先します。最後のサイドを取るKOも引き続き最優先です。
