...
class AlakazamPolicy:
    ...
    def _fez_mode(self, pokemon=None, *, for_bench=False):
        ...
        if pokemon is not None and pokemon.id == C.FEZANDIPITI_EX:
            if self._fez_alternate_matchup() and self._fez_energy_eta(pokemon) <= 2:
    ...
    def _fez_attach_score(self, pokemon, is_active):
        ...
        if mode == "ALTERNATE_ATTACKER" and self._fez_energy_eta(pokemon) <= 2:
            return 9300 + (500 if is_active else 0) + attached * 150
        return -1
