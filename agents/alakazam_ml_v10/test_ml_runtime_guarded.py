...
def test_evolution_candidates_are_excluded_from_v9_live_scope():
    fallback = ctx("evolve", 743)
    assert _candidate_scope_reason(ctx("evolve", 743), fallback) == "candidate_rule_only_evolve"
    assert _candidate_scope_reason(ctx("evolve", 66), fallback) == "candidate_rule_only_evolve"


def test_v10_semantic_key_collapses_interchangeable_hand_copies():
    first = ctx("bench", 741)
    second = ctx("bench", 741)
    first.update({"index": 2, "target_area": -1, "target_index": -1})
    second.update({"index": 7, "target_area": -1, "target_index": -1})
