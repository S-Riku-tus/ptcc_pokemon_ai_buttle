...
def test_sources_parse():
    for name in ("main.py","fallback_v12.py","policy_base.py","ml_runtime.py","ml_features.py","common_runtime.py"):
        ast.parse((ROOT/name).read_text())

def test_deck_revision():
    c=Counter(map(int,(ROOT/"deck.csv").read_text().split()))
    assert sum(c.values())==60
    assert c[305]==3 and c[66]==3 and c[858]==0 and c[5]==3

def test_role_cleanup():
    f=(ROOT/"fallback_v12.py").read_text(); m=(ROOT/"ml_runtime.py").read_text()
