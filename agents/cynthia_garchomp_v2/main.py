# cynthia_garchomp_v2 - v1 + 実ラダー敗戦(sub54521273, vs Crustle ep85178595)対策:
#   Crustle等の「exポケモンのワザのダメージを受けない」耐性を認識し、
#   (a) Garchomp exのワザは耐性持ちにダメージ0として計画
#   (b) ボスで耐性持ちを前に呼ばない
#   (c) 相手の前が耐性持ちの時は非exのスピリトム(Raging Curse)を主軸に切り替え
# Base: wmh/ptcg-abc agents/garchomp (nasuo445 divergence-mined).
...
# 「相手のex/Mega exポケモンのワザのダメージを受けない」耐性持ち
# (Crustle 345, Cornerstone Mask Ogerpon 330 など)。wmh/ptcg-abcの実測リスト。
EX_IMMUNE = {158, 207, 330, 345}
...
class GarchompPolicy(BasePolicy):
    ...
    def _atk_dmg(self, aid, target):
        ...
        if target is None or aid is None:
            return 0
        # v2: Garchomp ex のワザは「exのワザのダメージを受けない」耐性持ちに0。
        # (Raging Curseはスピリトム=非exなので通る)
        if target.id in EX_IMMUNE and aid in (CORKSCREW_DIVE, DRACONIC_BUSTER):
            return 0
    ...
    def gust_value(self, card):
        # v2: ボスで耐性持ち(Crustle等)を前に呼ぶのは自殺行為 — 強くペナルティ
        v = super().gust_value(card)
        if getattr(card, "id", None) in EX_IMMUNE:
            v -= 5000
        return v

    def _opp_active_immune(self):
        return self.opp is not None and self.opp.id in EX_IMMUNE
