"""Conservative deterministic fallback for exact-list Dragapult v2.
...
# training episodes: 1058; minimum offers per card: 25
TEACHER_DISCARD_RATE = {
    2: 0.1166,  # Basic {R} Energy 95/815
    5: 0.1464,  # Basic {P} Energy 100/683
    7: 0.2402,  # Basic {D} Energy 123/512
    112: 0.4689,  # Munkidori 128/273
    119: 0.5424,  # Dreepy 64/118
    120: 0.335,  # Drakloak 134/400
    121: 0.3106,  # Dragapult ex 191/615
    140: 0.2105,  # Fezandipiti ex 16/76
    235: 0.9525,  # Budew 301/316
    1071: 0.41,  # Meowth ex 82/200
    1080: 0.0172,  # Unfair Stamp 5/290
    1086: 0.8762,  # Buddy-Buddy Poffin 559/638
    1097: 0.1244,  # Night Stretcher 51/410
    1120: 0.305,  # Crushing Hammer 122/400
    1121: 0.6399,  # Ultra Ball 295/461
    1152: 0.4348,  # Poké Pad 110/253
    1182: 0.2944,  # Boss’s Orders 272/924
    1198: 0.1901,  # Crispin 150/789
    1213: 0.2725,  # Judge 94/345
    1227: 0.2742,  # Lillie's Determination 297/1083
    1231: 0.7807,  # Dawn 210/269
    1246: 0.2861,  # Jamming Tower 105/367
}
# --- END GENERATED DISCARD TABLE ---
...
class DragapultPolicy(BasePolicy):
    ...
    def score_discard(self, card):
        """Which cards to pay a cost with.

        Ultra Ball's two-card discard is a multi-pick select, so the ranker
        never scores it and no imitation metric covers it; the frozen test
        split put this context at 21.9% agreement, the worst of any.  The base
        order is therefore the teachers' own measured discard rate rather than
        a hand-written priority, with only two overrides on top: never pay with
        a piece the route still needs, and treat a spare copy as spare.
        """
        if card is None:
            return 0
        cid = card.id
        if cid == C.DARK and self.field[C.MUNKIDORI] and not any(
            ...
            return -10_000
        spare = max(0, self.hand[cid] - 1)
        if cid in (C.FIRE, C.PSYCHIC) and self.route_needs_energy() and not spare:
            return -8_000
        if cid == C.DRAKLOAK and self.field[C.DREEPY] > spare:
            return -7_000
        if cid == C.DRAGAPULT and self.field[C.DRAKLOAK] > spare:
            return -7_000
        if cid == C.DREEPY and self.count_line() < 2 and not spare:
            return -6_000
        rate = (
            TEACHER_DISCARD_RATE.get(cid)
            if self.context == SelectContext.DISCARD else None
        )
        if rate is None:
            # Unmeasured card: keep the old duplicates-first behaviour.
            return 4_000 + 500 * self.hand[cid] if self.hand[cid] >= 2 else 100
        return int(round(rate * 10_000)) + 800 * spare
    ...
    def ability_holder(self, option):
        """The body whose Ability an ABILITY option activates.

        OptionType.ABILITY documents ``area``/``index``; it has no
        ``inPlayArea``/``inPlayIndex``.  Reading the latter resolved every
        ability to None, so both branches below were dead and every ability
        scored the same generic constant.
        """
        return get_card(
            self.obs, option.area, option.index,
            getattr(option, "playerIndex", None) or self.my_index,
        )

    def score_ability(self, option):
        holder = self.ability_holder(option)
        if holder is None:
            return 700_000
        if holder.id == C.MUNKIDORI:
            # Adrena-Brain moves our own counters onto their board, so it is
            # only worth an action when we actually carry counters to move.
            movable = max(
                (min(3, (p.maxHp - p.hp) // 10) for p in self.my_board()),
                default=0,
            )
            return 930_000 if movable > 0 else -1
        if holder.id == C.DRAKLOAK:
            return 920_000 if self.me.deckCount > 1 else -1
        if holder.id == C.FEZANDIPITI:
            return 910_000
        return 700_000
    ...
    def score_spread_target(self, card):
        """Where one Phantom Dive damage counter goes.

        The engine asks for the counters one at a time and reports how many are
        still in hand, so "dies to this spread" is `hp <= remaining * 10`, not
        the fixed `hp <= 60` of a full six-counter swing.  With four counters
        already spent, a 60 HP body is no longer a kill and the counter is
        better spent setting up the next turn's kill on a two-prize body.
        """
        if not isinstance(card, Pokemon):
            return 0
        hp = int(card.hp)
        remaining = int(getattr(self.select, "remainDamageCounter", 0) or 0)
        budget = remaining * 10 if remaining > 0 else 60
        ko_bonus = 20_000 if 0 < hp <= budget else 0
