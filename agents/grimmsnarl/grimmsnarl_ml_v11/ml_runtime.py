...
class Ranker:
    ...
    def save_dynamic_state(self) -> dict[str, Any]:
        """Snapshot mutable per-game state without copying the 45 MB model.

        The arithmetic search advances the same ranker independently down
        several hypothetical turn lines.  Each branch must start from the
        identical offer/pass history and the live state must be restored before
        the real action is committed.
        """
