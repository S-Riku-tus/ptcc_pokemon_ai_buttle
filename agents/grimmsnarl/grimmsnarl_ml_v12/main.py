"""Marnie's Grimmsnarl ex v12: v9 ranker plus whole-turn arithmetic search.

v12 keeps every byte of v11.1's policy - the same deck, the same v9 ranker, the
same fallback, the same leaf arithmetic - and changes only how much of the turn
the search is allowed to see and how strictly its answer has to hold up.

**v11.1 searched 16.6% of the decisions it could have.**  Recomputed from its
own 59 ladder games, our own turns contain 6.02 MAIN decisions with a real
choice, and the once-per-turn rule spent the search on the first of them - the
widest, least decidable one - and then stood down for the rest of the turn.
That is 393 searches against 2,372 searchable decisions.  The ordering
decisions a whole-turn leaf is actually good at settling all sit in the 83.4%
it never looked at.

**It was not a cost decision, because there was no cost pressure.**  With
``actTimeout: 0`` the entire per-episode budget is the 600 s overage bank, and
v11.1's worst game handed back 579.5 s of it.  The layer used 3.4% of its
allowance.  v12 spends that budget: it searches every MAIN decision it can
afford, governed by ``remainingOverageTime`` and its own wall clock, with a
150 s reserve and a documented degradation ladder (full coverage -> once per
turn -> off) so a long game loses search rather than the game.

**More coverage is more exposure to a hand-written leaf, so the gate is
tighter in step.**  Three hidden-state determinizations instead of two, all
of which must agree, and a new invariant: a candidate may not end our turn
handing the opponent more prizes on their immediate reply than v9's line does,
unless it attacks or deals damage for them.  The extra sample is nearly free
because later samples only re-simulate candidates that already beat v9.

The v11.1 Alakazam-going-second special case is **removed**, not weakened.  It
existed to buy that matchup a second search per turn; under full coverage every
matchup gets every search, so a matchup-keyed branch would only be a
constraint.  Search failure, disagreement and ties still return v9.
