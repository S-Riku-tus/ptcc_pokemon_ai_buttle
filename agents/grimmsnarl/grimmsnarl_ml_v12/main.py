"""Marnie's Grimmsnarl ex v6 ML: imitation ranker plus a one-ply route planner.
...
_FRESH_RANKER: Ranker | None = None
_OLD20_RANKER: Ranker | None = None
_LOAD_ERROR: str | None = None
if os.environ.get("GRIMMSNARL_ML_DISABLE") != "1":
    try:
        _RANKER = Ranker()
        _FRESH_RANKER = Ranker("ranker_fresh0_model.json")
        _OLD20_RANKER = Ranker("ranker_old20_model.json")
        # The offline gate measured the old ranker at its native pin 20.  v6's
        # Froslass class route belongs to the old production policy and would
        # otherwise silently turn this voter into pin 0 on those menus.
        _OLD20_RANKER.escalation_code = None
...
def _rankers():
    return tuple(
        ranker for ranker in (_RANKER, _FRESH_RANKER, _OLD20_RANKER)
        if ranker is not None
    )


_GATE_STATS = {"considered": 0, "voters_agreed": 0, "overrides": 0}
...
def _choose(observation):
    ...
    fresh_index = (
        _FRESH_RANKER.choose(observation)
        if _FRESH_RANKER is not None else None
    )
    old20_index = (
        _OLD20_RANKER.choose(observation)
        if _OLD20_RANKER is not None else None
    )
    _GATE_STATS["considered"] += 1
    if index is None:
        ...
        if not _RANKER.teacher_forced:
            ...
            if _PLANNER is not None:
                _PLANNER.note(observation, select, chosen)
        return rule_choice
    # Validation-selected conservative gate: current pooled is the baseline.
    # Only two independent current-policy voters agreeing on the exact raw
    # candidate can replace it.  If either voter fails, baseline remains.
    if (
        fresh_index is not None
        and old20_index is not None
        and fresh_index == old20_index
    ):
        _GATE_STATS["voters_agreed"] += 1
        _GATE_STATS["overrides"] += int(fresh_index != index)
        index = fresh_index
...
def diag_reset():
    ...
    _GATE_STATS.update({"considered": 0, "voters_agreed": 0, "overrides": 0})
    for ranker in _rankers():
        ranker.reset()
    if _PLANNER is not None:
        _PLANNER.reset()
...
def diag_snapshot():
    return {
        ...
        "fresh_voter": (
            _FRESH_RANKER.snapshot() if _FRESH_RANKER is not None else {}
        ),
        "old20_voter": (
            _OLD20_RANKER.snapshot() if _OLD20_RANKER is not None else {}
        ),
        "consensus_gate": dict(_GATE_STATS),
