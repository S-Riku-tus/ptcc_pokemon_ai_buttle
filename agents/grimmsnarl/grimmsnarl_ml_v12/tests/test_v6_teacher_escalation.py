...
def test_metadata_records_unconditioned_v9_lineage() -> None:
    ...
    The parent agent moved on (v9 -> v11 -> v12) while the ranker did not, so
    the claim is pinned to ``ranker_lineage`` rather than to ``parent_agent``.
    """
    meta = json.loads((AGENT_DIR / "metadata.json").read_text(encoding="utf-8"))
    assert meta["ranker_lineage"] == "grimmsnarl_ml_v9"
