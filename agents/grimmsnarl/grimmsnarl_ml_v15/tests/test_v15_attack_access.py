...
def test_v15_changes_exactly_one_module_against_v14() -> None:
    ...
    v14 = AGENT_DIR.parent / "grimmsnarl_ml_v14"
    if not v14.is_dir():  # v14 may be archived away later
        return
    for name in metadata["files_identical_to_v14"]:
        ours = hashlib.sha256((AGENT_DIR / name).read_bytes()).hexdigest()
        theirs = hashlib.sha256((v14 / name).read_bytes()).hexdigest()
