...
# ----- artifacts ------------------------------------------------------------
def test_v16_changes_exactly_two_things_against_v15() -> None:
    """v15's own safety argument, re-asserted against v15 rather than v14.
    ...
    assert metadata["name"] == "grimmsnarl_ml_v16"
    assert metadata["deck_hash"] == "9714ab5c3996f6cc"
    assert metadata["deck_changed"] is False
    assert metadata["ranker"]["model_changed"] is False
    ...
    # Exactly one new module, and exactly two edited files.
    changed = set(metadata["files_changed_from_v15"])
    assert changed == {"main.py", "ml_runtime.py", "wall_break.py"}
    assert not (v15 / "wall_break.py").exists()
    for name in changed - {"wall_break.py"}:
        ours = hashlib.sha256((AGENT_DIR / name).read_bytes()).hexdigest()
        theirs = hashlib.sha256((v15 / name).read_bytes()).hexdigest()
        assert ours != theirs, name
    assert (
