...
def test_v17_changes_only_the_files_declared_against_v16() -> None:
    metadata = json.loads(
        (AGENT_DIR / "metadata.json").read_text(encoding="utf-8")
    )
    v16 = AGENT_DIR.parent / "grimmsnarl_ml_v16"
    assert v16.is_dir()

    for name in metadata["files_identical_to_v16"]:
        ours = hashlib.sha256((AGENT_DIR / name).read_bytes()).hexdigest()
        theirs = hashlib.sha256((v16 / name).read_bytes()).hexdigest()
        assert ours == theirs, name

    for name in metadata["files_changed_from_v16"]:
        ours = hashlib.sha256((AGENT_DIR / name).read_bytes()).hexdigest()
        theirs = hashlib.sha256((v16 / name).read_bytes()).hexdigest()
        assert ours != theirs, name
