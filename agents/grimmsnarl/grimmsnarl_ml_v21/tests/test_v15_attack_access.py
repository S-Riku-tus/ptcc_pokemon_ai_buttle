...
def test_voluntary_retreat_cannot_pass_a_ready_promotion_after_energy_costs() -> None:
    """Regression for submitted v19 episode 91950626, turn 6.

    The engine inserts one or more context-30 Energy-discard selects between
    RETREAT and context-3 promotion.  Those micro-selects must not erase the
    pending ready-attacker invariant.
    """
    guard = AttackAccessGuard()
    active = poke(GRIM, energy=2, hp=320, serial=87)
    ready = poke(GRIM, energy=2, hp=320, serial=88)
    morgrem = poke(MORGREM, energy=3, hp=100, serial=85)
    retreat = observation(
        active=active,
        bench=[morgrem, ready],
        hand=[],
    ...
    guard.note(retreat)
    assert guard.adjust(retreat, retreat["select"], 1, [1]) == 1
    ...
    discard = observation(
        ...
        hand=[],
        context=30,
        options=[{"type": 6}, {"type": 6}],
        turn=6,
    )
    guard.note(discard)
    assert guard.adjust(discard, discard["select"], 0, [0]) == 0
    guard.note(discard)
    assert guard.adjust(discard, discard["select"], 0, [0]) == 0

    promote = observation(
        active=poke(GRIM, energy=0, hp=320, serial=87),
        bench=[morgrem, ready],
        hand=[],
        context=mf.CTX_SWITCH,
        options=[
...
def test_v21_keeps_the_deck_and_attack_access_against_v15() -> None:
    ...
    assert metadata["name"] == "grimmsnarl_ml_v21"
    ...
    assert changed == {
        ...
        "ml_features.py", "horizon_prize.py",
    }
    assert not (v15 / "wall_break.py").exists()
    assert not (v15 / "mirror_prize.py").exists()
    for name in changed - {
        "wall_break.py", "mirror_prize.py", "horizon_prize.py"
    }:
        ours = hashlib.sha256((AGENT_DIR / name).read_bytes()).hexdigest()
        theirs = hashlib.sha256((v15 / name).read_bytes()).hexdigest()
        assert ours != theirs, name
