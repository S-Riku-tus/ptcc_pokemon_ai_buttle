"""Grimmsnarl ML v23: phase-routed imitation with two measured state goals.

V23 keeps v22's 1220-pilot policy from own turn three onward, restores the
more reliable v8 pilot condition for own turns one and two, and adds a narrow
goal layer for first-attacker access and Alakazam backup continuity.
"""
...
from v23_strategy import StrategyPlanner
...
try:
    _STRATEGY = StrategyPlanner()
    _STRATEGY_ERROR: str | None = None
except Exception as error:  # noqa: BLE001
    _STRATEGY = None
    _STRATEGY_ERROR = f"{type(error).__name__}: {error}"
_STRATEGY_DISABLED = (
    _STRATEGY is None or os.environ.get("GRIMMSNARL_V23_STRATEGY_DISABLE") == "1"
)
...
def _choose(observation):
    ...
    if not _STRATEGY_DISABLED:
        index = _STRATEGY.adjust(
            observation,
            select,
            index,
            _RANKER.last_scores,
            _RANKER.pending_features,
        )
...
def diag_snapshot():
    return {
        ...
        "strategy": _STRATEGY.snapshot() if _STRATEGY is not None else {},
        "load_error": _LOAD_ERROR,
        "planner_load_error": _PLANNER_ERROR,
        "strategy_load_error": _STRATEGY_ERROR,
    }
