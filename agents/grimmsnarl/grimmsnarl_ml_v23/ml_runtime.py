...
# v23 uses the v8 pilot only while the first board is being built.  V22's
# 1220-rated pilot remains the default stored in ranker_model.json and owns all
# decisions from own turn three onward.
OPENING_TEACHER_TEAM = 16494330
OPENING_TEACHER_CODE = 16
V22_TEACHER_TEAM = 16371703
V22_TEACHER_CODE = 0
OPENING_LAST_OWN_TURN = 2
...
class Ranker:
    ...
    def reset(self) -> None:
        ...
        self.stats = {
            ...
            "opening_teacher_scored": 0,
            "v22_teacher_scored": 0,
        }
    ...
    def choose(self, observation: dict[str, Any]) -> int | None:
        ...
        decision_teacher_code = self._teacher_code_for(observation)
        if decision_teacher_code == OPENING_TEACHER_CODE:
            self.stats["opening_teacher_scored"] += 1
        else:
            self.stats["v22_teacher_scored"] += 1
        ...
        try:
            if spec is not None and self.escalation_mode == "class":
                ...
                pinned_index, _ = self._score(
                    features, representatives, decision_teacher_code
                )
    ...
    def own_turn(current: dict[str, Any]) -> int:
        """One-based turn number for the current seat; setup is turn zero."""
    ...
    def _teacher_code_for(self, observation: dict[str, Any]) -> Any:
        turn = self.own_turn(observation.get("current") or {})
        if 1 <= turn <= OPENING_LAST_OWN_TURN:
            return OPENING_TEACHER_CODE
    ...
    @property
    def pending_features(self) -> list[dict[str, Any]] | None:
        """Feature rows for the current choice, before ``commit`` clears them."""
