...
def test_v25_deployed_model_is_globally_pinned_to_alphatcg() -> None:
    """The AlphaTCG pin must not reactivate the legacy class escalation."""
    ...
    assert int(match.group(1)) == DEPLOYED_PIN_CODE
...
def test_metadata_records_the_escalation() -> None:
    ...
    assert escalation["mode"] == ml_runtime.ESCALATION_MODE == "off"
    assert escalation["legacy_teacher_team_id"] == ml_runtime.ESCALATION_TEACHER_TEAM
    assert escalation["legacy_teacher_code"] == ELITE_CODE
    assert meta["change_scope"]["v24_froslass_veto_included"] is False
