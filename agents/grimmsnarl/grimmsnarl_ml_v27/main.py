"""Grimmsnarl ML v27: v22 champion plus belief-corrected adaptive H3.

v22 remains the default policy.  v25's compact AlphaTCG model is loaded only
as a root candidate and alternate opponent response in a publicly confirmed
mirror; it never owns the final action.  The Search API first compares three
legal without-replacement belief worlds through H2, and extends only close,
tactical, sufficiently observable positions through H3 with adaptive samples.
The measured v24 mirror-Froslass veto and v26 wall/deck guards remain narrow
...
_MIRROR_FROSLASS, _MIRROR_FROSLASS_ERROR = _optional(
    "mirror_froslass", "MirrorFroslassGuard"
)
...
def _choose(observation):
    ...
    mirror_froslass_index = index

    # The statistical planner is allowed only in exact public mirrors.  It
    # receives the deterministic layers' answer.  Only the measured v24
    # Froslass veto may supply a non-ranker-top baseline for counterfactual
    # comparison; other one-ply planner invariants remain final.
    if _H2 is not None and base_pending:
        index = _H2.adjust(
            ...
            opponent_ranker=_PEER,
            allow_non_top=(
                mirror_froslass_index != planner_index
                and planner_index == base_index
            ),
        )
    h2_index = index
    ...
    _LAST_TRACE.update({
        ...
        "mirror_froslass": mirror_froslass_index,
        "h2": h2_index,
...
def diag_reset():
    ...
    for component in (
        ...
        _WALL_BREAK, _DECK_CLOCK, _H2, _MIRROR_FROSLASS,
    ):
...
def diag_snapshot():
    return {
        ...
        "mirror_froslass": _snapshot(_MIRROR_FROSLASS),
        ...
        "load_errors": {
            ...
            "h2_search": _H2_ERROR,
            "mirror_froslass": _MIRROR_FROSLASS_ERROR,
        },
    }
