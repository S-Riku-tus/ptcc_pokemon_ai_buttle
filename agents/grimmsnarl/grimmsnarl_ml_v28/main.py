"""Grimmsnarl ML v28: AlphaTCG race policy with a v22 wall specialist.
...
was 27-23.  v28 therefore keeps both rankers synchronized and switches to v22
as soon as a public wall card or Neutralization Zone identifies that route.

This is intentionally a policy-sized change.  v26/v27 changed only eight
decisions per run and their search never overrode v22; v28 removes the dead
search and the refuted mirror-Froslass veto.  The inherited deterministic wall
and deck-clock guards remain after the selected ranker and one-ply planner.
"""
...
def _policy_name(route: str) -> str:
    ...
    forced = os.environ.get("GRIMMSNARL_V28_POLICY", "auto").lower()
    if forced in {"v22", "wall"}:
        return "v22"
    if forced in {"v25", "race"}:
        return "v25"
    return "v22" if route == "v8_wall_guarded" else "v25"
