...
def test_v26_ports_the_measured_v21_wall_breaker_unchanged() -> None:
    """The wall fix is a proven component, not a fresh rewrite."""
    v21 = AGENT_DIR.parent / "grimmsnarl_ml_v21"
    assert v21.is_dir()
    ours = hashlib.sha256((AGENT_DIR / "wall_break.py").read_bytes()).hexdigest()
    theirs = hashlib.sha256((v21 / "wall_break.py").read_bytes()).hexdigest()
    assert ours == theirs
