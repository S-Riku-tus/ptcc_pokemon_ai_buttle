...
def test_shipped_mode_is_the_one_the_results_report_measured() -> None:
    # v28's default ranker is v25/AlphaTCG.  Class escalation would silently
    # restore the observationally-refuted Froslass veto, so it stays off.
    assert ml_runtime.ESCALATION_MODE == "off"
    assert ml_runtime.ESCALATION_TEACHER_TEAM == 16371703


def test_v28_default_model_is_alphatcg_and_escalation_is_disabled() -> None:
    ...
    assert ml_runtime.ESCALATION_TEACHER_CODE == DEPLOYED_PIN_CODE
