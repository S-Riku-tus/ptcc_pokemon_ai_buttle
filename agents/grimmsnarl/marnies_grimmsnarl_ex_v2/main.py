"""Marnie's Grimmsnarl ex v2: dual-leader route and lock-aware policy.

The 60-card list remains unchanged.  This final v2 combines 908 upper-ladder
replays from two independent pilots.  It preserves the preliminary v2's attack
reservation and coordinated 30-point prize map while adding direct-Candy route
recognition, earlier Munkidori activation, lock-aware Shadow Bullet damage, and
broader matchup role priorities.
"""
...
class GrimmsnarlPolicy(BasePolicy):
    ...
    def reserve_adjust(self, card_id: int, score: int) -> int:
        ...
        if card_id == C.BOSSES_ORDERS and self.best_boss_value() >= 10_000:
            return score
        if card_id == C.UNFAIR_STAMP and not self.active_ko_ready() and self.opponent.handCount >= 6:
            return min(score, 755_000)
    ...
    def score_play_poke(self, card):
        ...
        if card.id == C.IMPIDIMP:
            ...
            # One powered Munkidori is the normal target; the second is only for a developed board.
            score = (695_000 + safety) if self.field[C.MUNKIDORI] == 0 else (525_000 if self.field[C.MUNKIDORI] == 1 and self.board_count() >= 4 else -1)
        else:
            return -1
    ...
    def score_play_trainer(self, card):
        ...
        if cid == C.RARE_CANDY:
            ...
            score = 875_000 if self.opponent.handCount > 2 or self.me.handCount < 5 else 610_000
