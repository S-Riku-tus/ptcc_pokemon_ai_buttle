"""Marnie's Grimmsnarl ex v4: route-aware pre-attack sequencing.

The 60-card list remains unchanged.  v4 turns the v3 analysis into executable
policy constraints instead of adding more matchup-only score bumps:

* safe actions that preserve the live Shadow Bullet (a useful second Munkidori,
  a complete Rare-Candy backup route, or a profitable Unfair Stamp) may happen
  before the attack;
* optional searches and third-line development stay below the attack;
* Candy and search decisions share the same route-deficit model, so an already
  complete Impidimp-Candy-Grimmsnarl route no longer asks for Morgrem;
* Unfair Stamp compares both hands and protects a unique backup route;
* spread/counter targets include a one-turn prize-route value, preferring a
  reachable two-prize target over an isolated low-value counter placement.
...
class GrimmsnarlPolicy(BasePolicy):
    ...
    def score_play_poke(self, card):
        ...
        if card.id == C.IMPIDIMP:
            ...
            return -1
        if self.active_shadow_ready() and not (card.id == C.MUNKIDORI and self.useful_second_munkidori()):
    ...
    def score_ability(self, option):
        ...
        if source.id == C.SPIKEMUTH_GYM:
            ...
            if self.active_shadow_ready() and self.backup_is_close():
                return min(score, 730_000)
            return score
        return -1
