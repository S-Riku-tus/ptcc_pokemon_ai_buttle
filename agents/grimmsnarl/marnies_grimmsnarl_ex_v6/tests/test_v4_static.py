...
def test_runtime_is_self_contained():
    ...
    assert "from policy_base import" in (ROOT / "main.py").read_text(encoding="utf-8")


def test_initial_active_prefers_impidimp_then_munkidori_then_snorunt():
    # v6 change: the ladder log shows an Impidimp start is fastest/best and a
    # Snorunt start slowest, so the v4/v5 "preserve the only Impidimp" heuristic
    # is dropped for the data-backed order Impidimp > Munkidori > Snorunt.
    main = load_main()
