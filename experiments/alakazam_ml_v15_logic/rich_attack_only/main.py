"""Ablation: use Rich only on a turn where a real attack is already offered."""
...
spec = importlib.util.spec_from_file_location("_v14_rich_attack", V14 / "fallback_policy.py")
...
def _enriching_attach_score(self, pokemon, *, is_active=False):
    if not any(option.type == P.OptionType.ATTACK for option in (self.select.option or [])):
        return -1
