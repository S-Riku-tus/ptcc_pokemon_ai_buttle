...
v19 replaces the old one-pilot pin with a learned matchup-conditional pin.
Every option inside one argmax is still scored by exactly one pilot function;
the public router may change that function only when the opponent archetype
becomes visible.  The mapping is embedded in the trained model artifact.
...
# "off"     - v5 exactly.
# v19's refreshed model is already pinned to the selected current high-rated
# teacher for each public matchup.  Re-applying v6's legacy class escalation
# would splice an old 2026-08-05 pilot back into the new learned policy.
ESCALATION_MODE = "off"
...
class Ranker:
    def __init__(self, model_path: str = "ranker_model.json"):
        ...
        self.route_teacher_codes = {
            str(route): int(code) for route, code in
            (self.model.get("route_teacher_codes") or {}).items()
        }
        self.teacher_index = (
    ...
    def set_route(self, route: str) -> None:
        """Select one trained pilot for this public matchup.

        This changes only the categorical teacher code supplied to the shared
        ranker.  It never blends candidate scores from different teachers.
        """
        ...
        if route != self.active_route:
            ...
            if self.active_route and code != self.teacher_code:
