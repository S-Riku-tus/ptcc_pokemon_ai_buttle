...
class WallBreakGuard:
    ...
    def _spends_last_breaker(
        ...
        """This evolution consumes the only body that can hit the wall.
        ...
        if not ready:
            return False
        # The evolution has to be the one landing on that last breaker.
        ...
        if not 0 <= slot < len(pool):
            return False
        return pool[slot].get("serial") == breakers[0].get("serial")
