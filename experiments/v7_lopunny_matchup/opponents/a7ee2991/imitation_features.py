...
# never used.
KEY_CARD_IDS = [
    3, 5, 11, 13, 14, 19, 66, 104, 112, 140, 174, 305, 343, 648, 741,
    742, 743, 848, 849, 1079, 1081, 1086, 1097, 1121, 1122, 1129,
    860, 861, 1087, 1152, 1174, 1182, 1197, 1225, 1227, 1229, 1231,
    1264, 1266,
]
ENERGY_IDS = {3, 5, 11, 13, 14, 17, 19}
PSYCHIC_ENERGY_IDS = {5, 19}
BASIC_IDS = {66, 140, 174, 305, 343, 741, 848, 860}
...
def state_features(current: dict[str, Any]) -> dict[str, float | int]:
    ...
    out.update({
        ...
        "snorunt_ex_route_count": field_counts[SNORUNT_EX_LINE_ID],
        "mega_froslass_ex_route_count": field_counts[MEGA_FROSLASS_EX_ID],
        "has_snorunt_ex_in_hand": int(hand_counts[SNORUNT_EX_LINE_ID] > 0),
        "has_mega_froslass_ex_in_hand": int(
            hand_counts[MEGA_FROSLASS_EX_ID] > 0
        ),
        "ready_mega_froslass_ex_count": sum(
            int(
                int(card.get("id", -1)) == MEGA_FROSLASS_EX_ID
...
def option_features(
    ...
    out.update({
        ...
        "candidate_is_snorunt_ex": int(card_id == SNORUNT_EX_LINE_ID),
        "candidate_is_mega_froslass_ex": int(
            card_id == MEGA_FROSLASS_EX_ID
        ),
        ...
        "target_is_snorunt_ex": int(target_id == SNORUNT_EX_LINE_ID),
        "target_is_mega_froslass_ex": int(
            target_id == MEGA_FROSLASS_EX_ID
        ),
