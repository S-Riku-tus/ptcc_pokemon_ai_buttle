...
KNOWN_ACE_SPEC_IDS = {C.ENRICHING_ENERGY}
...
class AlakazamPolicy(BasePolicy):
    ...
    def _fez_draw_needed(self):
        return (self._ko_during_previous_opponent_turn()
                and self.me.handCount < 10
                and self.me.deckCount >= max(4, self._turns_to_win() + 2)
                and not self._plan.get("kos"))
    ...
    def _fezandipiti_worthwhile(self):
        if self.field[C.FEZANDIPITI_EX] > 0 or not self._open_bench():
            return False
        mode = self._fez_mode()
        if mode == FezMode.DRAW_ONLY:
            return self.me.handCount < 10 and self.me.deckCount >= 4 and not self._plan.get("kos")
        return mode == FezMode.ALTERNATE_ATTACKER
    ...
    def _search_card_has_goal(self, cid):
        ...
        if self._opening_window():
            if cid == C.BUDDY_POFFIN:
                return missing_abra or missing_engine or self._shaymin_worthwhile()
            if cid == C.POKE_PAD:
                return missing_abra or missing_engine or self._need_pieces()
            if cid == C.DAWN:
                return missing_abra or self._need_pieces()
            if cid == C.HILDA:
                return self._energy_starved() or self._need_pieces()
        if not self._ready_alakazam_attacker():
            return cid in (C.POKE_PAD, C.HILDA, C.DAWN, C.BUDDY_POFFIN)
    ...
    def _boss_target_score(self, target):
        ...
        two_prize = prizes >= 2
        protector = target.id in GLOBAL_EFFECT_PROTECTORS
    ...
    def _score_play_poke(self, card):
        ...
        if cid == C.FEZANDIPITI_EX:
            return 18800 if self._fezandipiti_worthwhile() else -1
        return 14000 - 200 * n
    ...
    def _score_to_bench(self, card):
        ...
        if cid == C.FEZANDIPITI_EX:
            return 240 if self._fezandipiti_worthwhile() else -1
        return 100 - 20 * n
