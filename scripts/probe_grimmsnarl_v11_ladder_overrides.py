"""Attribute the v11 ladder result to the arithmetic-search layer, per game.

The v11 report established the override *rate* on teacher replays. That cannot
say whether the layer helped, because a teacher replay has no v11 outcome. This
walks v8 and v11 side by side over v11's **own** ladder games - the states v11
actually reached - and joins each override to the result of the game it was
played in.
...
def main() -> int:
    ...
    report = {
        ...
        "reproduction": {
            ...
            "v8_matches_played": reproduced["v8"],
            "v11_matches_played": reproduced["v11"],
            "v8_rate": (
                round(reproduced["v8"] / reproduced["decisions"], 4)
                if reproduced["decisions"] else None
            ),
            "v11_rate": (
                round(reproduced["v11"] / reproduced["decisions"], 4)
                if reproduced["decisions"] else None
            ),
            "note": (
                "v11 is the deployed policy, so its rate is the probe's "
                "fidelity ceiling. A rate well below 1.0 means the replayed "
                "layer is not reproducing deployed behaviour and the ledger "
                "under-counts."
            ),
        },
        ...
        "cards_v8_would_play": dict(Counter(
            (row["v8"] or {}).get("name", "?") for row in ledger
        ).most_common()),
        "cards_v11_plays": dict(Counter(
            (row["v11"] or {}).get("name", "?") for row in ledger
        ).most_common()),
