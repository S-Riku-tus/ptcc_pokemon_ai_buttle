"""Teacher-forced v22/v26 comparison on both v25 ladder submissions.
...
ranker history.  The report therefore measures exactly where v26's wall and
deck-clock guards would intervene on real reachable boards, including the
critical invariant that ordinary non-wall boards remain v22.
"""
...
def run_agent(agent_dir: Path, episodes) -> tuple[
    ...
    for name in (
        ...
        "value_features", "wall_break", "wall_trajectory",
    ):
    ...
    # time without changing any v26 choice.
    if agent_dir.name == "grimmsnarl_ml_v26":
        module._PEER = None
    ...
    for episode in episodes:
        ...
        if hasattr(module, "diag_snapshot"):
            ...
            for section in ("wall_trajectory", "wall_break", "deck_clock"):
...
def main() -> int:
    ...
    v26, v22, guard_stats = run_agent(
        ROOT / "agents/grimmsnarl/grimmsnarl_ml_v26", episodes
    )
    counts = Counter()
    differences = []
    for episode in episodes:
        ...
        for offset, decision in enumerate(episode["decisions"]):
            if not decision.get("evaluate"):
                continue
            counts["decisions"] += 1
            changed = v22[episode_id][offset] != v26[episode_id][offset]
            cell = (
    ...
    target = ROOT / "experiments/grimmsnarl_ml_v26/replay_probe.json"
