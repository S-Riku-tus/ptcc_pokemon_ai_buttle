"""Train the v29 ranker from rank-weighted current Alakazam teacher replays.

The model is a residual policy: every candidate receives the full observable
board/option features plus v28's deterministic choice, deterministic score,
and the legacy ranker score.  Labels collapse interchangeable copies of the
same card.  Evaluation is episode-held-out and reports model, fallback, Top-k,
oracle, and confidence-gated hybrid agreement.
"""
...
def _split_for_episode(episode_id: int) -> str:
    bucket = int(
        hashlib.sha1(str(episode_id).encode("ascii")).hexdigest(), 16
    ) % 10
    if bucket < 2:
        return "test"
    if bucket == 2:
...
def _extract_chunk(
    ...
    fallback_agent = main_module._fallback_agent
    ...
    for row in rows:
        ...
        outcome_weight = 1.05 if won else 0.97
        decision_weight = (1.0 / math.sqrt(max(1, rank))) * outcome_weight
        ...
        for step_index, step in enumerate(steps[:-1]):
            ...
            try:
                fallback = list(fallback_agent(observation))
            except Exception:
                stats["fallback_error"] += 1
                continue
            if len(fallback) != 1 or not 0 <= fallback[0] < len(options):
            ...
            for option in options:
                feature = dict(
                    features_module.option_features(current, select, option)
                )
            ...
            fallback_key = _semantic_key(raw_features[fallback[0]])
            ...
            fallback_action_type = int(
                raw_features[fallback[0]]["action_type"]
            )
            fallback_card_id = int(
                raw_features[fallback[0]].get("candidate_card_id", -1)
            )
            fallback_legacy_agree = int(fallback[0] == legacy_order[0])
            ...
            for index, feature in enumerate(raw_features):
                ...
                if list(feature.keys()) != feature_names:
                    raise RuntimeError("v29 feature order changed within corpus")
            ...
            row_weights.extend([decision_weight] * len(options))
            groups.append(len(options))
            splits.append(_split_for_episode(episode_id))
            fallback_correct.append(int(fallback_key == teacher_key))
...
def main() -> int:
    ...
    ranks_filter = {int(value) for value in args.ranks.split(",") if value}
    with (args.run_root / "indexes" / "replay_index.csv").open(
        encoding="utf-8-sig", newline=""
    ) as handle:
        replay_rows = [
            row
            for row in csv.DictReader(handle)
            if int(row["leaderboard_rank"]) in ranks_filter
    ...
    if worker_count == 1:
        ...
        with ProcessPoolExecutor(max_workers=worker_count) as executor:
            parts = list(
                executor.map(
                    _extract_chunk,
                    [str(args.run_root.resolve())] * worker_count,
    ...
    report = {
        ...
        "label_definition": (
            "all legal options with identical card/attack and target state are positive"
        ),
