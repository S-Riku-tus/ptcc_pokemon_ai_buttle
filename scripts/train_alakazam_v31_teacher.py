"""Train the v31 coherent-teacher ranker from the expanded Alakazam corpus.

v31 starts from the v30 MAIN extraction and makes teacher-cohort selection
explicit so that policy disagreement is measured before models are blended.
The base extractor adds
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        default=[],
        help=(
            "Only use these source_cohort values. May be supplied more than "
            "once. The default uses every cohort."
        ),
    )
    parser.add_argument(
        "--cache-output",
        type=Path,
        help=(
            "Optionally persist the extracted candidate arrays as a compressed "
            "NumPy archive for v31 model experiments."
        ),
    )
    ...
    if args.include_cohort:
        allowed_cohorts = set(args.include_cohort)
        replay_rows = [
            row
            for row in replay_rows
            if row["source_cohort"] in allowed_cohorts
        ]
    if (
    ...
    compact.update({
        ...
        "runtime_scope": "v31_coherent_teacher_main_policy",
